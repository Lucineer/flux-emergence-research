...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int cluster_size, unsigned int seed) {
    ...
    if(cluster_size==0){
        // Fully random
        x = cr(&rng)*w; y = cr(&rng)*w;
    } else {
        // Cluster: agents in groups of cluster_size, each group at random point
        int group = tid / cluster_size;
        unsigned int grng = seed + group * 7777;
        float cx = cr(&grng)*w, cy = cr(&grng)*w;
        x = fmodf(cx + cr(&rng)*20.0f + w, w);
        y = fmodf(cy + cr(&rng)*20.0f + w, w);
    }
...
int main(){
    int clusters[]={0,1,4,16,64};
    const char* nm[]={"Random","AllCenter","Clust4","Clust16","Clust64"};
    printf("=== CLUSTER DENSITY: Law 257 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,clusters[m],(unsigned int)(42+tr*1111+m*111));
    ...
    printf("\n>> Law 257: Optimal spatial clustering for scripted agents\n");
