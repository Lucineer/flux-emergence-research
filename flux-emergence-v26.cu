/* flux-emergence-v26.cu — Adaptive Detection Range
   Novel: explorers (arch 0) widen their detection radius when nearby resources
   are depleted. Tests if dynamic sensing beats fixed detection.
   Prediction: adaptive sensing finds resources 10% faster. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int adaptive){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    
    /* v26: adaptive detection — count nearby resources, widen if few */
    float det=.03f+ep*.04f;
    if(adaptive&&ag->arch==0){ /* explorers only */
        int nearby=0;
        for(int j=0;j<nr;j++){if(r[j].collected)continue;float dx=r[j].x-ag->x,dy=r[j].y-ag->y;if(sqrtf(dx*dx+dy*dy)<.1f)nearby++;}
        /* If no resources nearby, double detection range */
        if(nearby==0)det*=2.0f;
    }
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v26 — Adaptive Detection Range\n");
    printf("  Explorers double detection when no nearby resources\n");
    printf("  A: adaptive  B: fixed  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_ad+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_fix+=f;
        ...
        printf("Exp %d: adaptive=%.1f fixed=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_ad/=5;f_fix/=5;f_ctrl/=5;
    float r_ad=(f_ctrl>.01f)?f_ad/f_ctrl:1,r_fix=(f_ctrl>.01f)?f_fix/f_ctrl:1;
    float boost=(f_fix>.01f)?f_ad/f_fix:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Adaptive: %.1f (%.2fx ctrl)\n",f_ad,r_ad);
    printf("  Fixed:    %.1f (%.2fx ctrl)\n",f_fix,r_fix);
    printf("  Control:  %.1f\n",f_ctrl);
    printf("  Boost:    %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"ADAPTIVE WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
