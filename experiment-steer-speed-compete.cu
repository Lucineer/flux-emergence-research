...
__global__ void simulate(float *scores_a, float *scores_b, float *fx, float *fy, int *falive,
    ...
    int a_steer, int b_steer, unsigned int seed) {
    ...
    // Fleet A
    {
        ...
        for(int t=0;t<steps&&energy>0;t++){
            ...
            if(a_steer){
                ...
                dx+=sx*speed_mult;dy+=sy*speed_mult;
...
int main(){
    ...
    for(int sp=0;sp<NSPEEDS;sp++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_sa,d_sb,d_fx,d_fy,d_fa,speeds[sp],STEPS,NF,FOOD,W,1,0,(unsigned int)(42+tr*1111+sp*111));
        ...
        ta/=TRIALS;tb/=TRIALS;
        printf("Speed %2.0fx: Steer=%.3f Pure=%.3f ratio=%.2f\n",speeds[sp],ta,tb,ta/(tb+0.001f));
    }
    printf("\n>> Law 243: Does steering competitive advantage hold at high speed?\n");
