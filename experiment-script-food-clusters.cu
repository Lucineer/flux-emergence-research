...
int main(){
    const char* nm[]={"Uniform","Clusters","Line"};
    printf("=== FOOD DISTRIBUTION: Law 224 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int pat=0;pat<NPATTERNS;pat++){
        ...
        } else if(pat==1){ // 4 clusters at corners of inner square
            for(int i=0;i<FOOD;i++){
                int c=i%4;
                float cx=(c%2==0)?64.0f:192.0f;
                float cy=(c<2)?64.0f:192.0f;
                hfx[i]=cx+((float)rand()/RAND_MAX)*60.0f;
                hfy[i]=cy+((float)rand()/RAND_MAX)*60.0f;
            }
        } else { // diagonal line
            for(int i=0;i<FOOD;i++){
                float t=(float)i/FOOD;
                hfx[i]=t*(W-32.0f)+16.0f+((float)rand()/RAND_MAX)*8.0f-4.0f;
                hfy[i]=t*(W-32.0f)+16.0f+((float)rand()/RAND_MAX)*8.0f-4.0f;
            }
        }
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,(unsigned int)(42+tr*1111+pat*111));
    ...
    printf("\n>> Law 224: Food distribution effect on scripted agents\n");
