/* flux-emergence-v43.cu — Optimal Cluster Count Sweep
   Novel: v25 used 4 clusters (one per archetype). What's optimal?
   Sweep 2, 4, 8, 16 clusters and measure fitness.
   Prediction: too few clusters = agents crowd each other; too many = no cluster benefit. */
...
__global__ void init_a_cluster(Agent*a,int n,int n_clusters){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    a[i].rng=(unsigned int)(i*2654435761u+17);a[i].arch=i%N_ARCH;
    int c=i%n_clusters;
    float angle=2.0f*3.14159265f*c/n_clusters;
    float cx=.5f+.35f*cosf(angle),cy=.5f+.35f*sinf(angle);
    a[i].x=cx+(lcgf(&a[i].rng)-.5f)*.15f;
    a[i].y=cy+(lcgf(&a[i].rng)-.5f)*.15f;
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v43 — Optimal Cluster Count Sweep\n");
    printf("  Test 1(uniform), 2, 4, 8, 16, 32 clusters\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    f_ctrl/=5;
    printf("Control: %.1f\n\n",f_ctrl);
    int clusters[]={1,2,4,8,16,32};
    for(int ci=0;ci<6;ci++){
        float f=0;
        for(int e=0;e<5;e++){
            if(clusters[ci]==1)init_a_uniform<<<blk,256>>>(da,NA);
            else init_a_cluster<<<blk,256>>>(da,NA,clusters[ci]);
            init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        ...
        printf("  %2d clusters: %.1f (%.2fx ctrl)\n",clusters[ci],f,ratio);
    }
    printf("\n═══════════════════════════════════════════════════════\n");
