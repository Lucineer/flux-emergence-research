...
__global__ void decay_traces(int *traces, int total, float factor) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= total) return;
    traces[i] = (int)(traces[i] * factor);
}
...
int main(){
    float decays[]={1.0f,0.5f,0.1f,0.01f,0.0f};
    const char* nm[]={"NoDecay","Decay0.5","Decay0.1","Decay0.01","Wiped"};
    printf("=== NOISE TRACE DECAY: Law 263 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",NF,FOOD,1500,TRIALS);
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            decay_traces<<<tblk,BLK>>>(d_tr,GRID*GRID,decays[m]);
            cudaDeviceSynchronize();
    ...
    printf("\n>> Law 263: Does trace decay help or hurt?\n");
