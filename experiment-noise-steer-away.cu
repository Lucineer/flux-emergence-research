    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(mode!=0){
            ...
            for(int dy2=-2;dy2<=2;dy2++)for(int dx2=-2;dx2<=2;dx2++){
                ...
                float sign = (mode==1) ? 1.0f : -1.0f; // +1=toward, -1=away
                sx+=sign*dx2*(float)v;sy+=sign*dy2*(float)v;
            }
...
int main(){
    const char* nm[]={"Ignore","Toward","Away"};
    printf("=== NOISE STEER DIRECTION: Law 260 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",NF,FOOD,1500,TRIALS);
    ...
    printf("\n>> Law 260: Steer toward vs away from noise traces\n");
