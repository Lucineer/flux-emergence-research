/* flux-emergence-v32.cu — Environmental Gradient (Rich Zone)
   Novel: 70% of resources spawn in a "rich zone" (x: 0.2-0.5, y: 0.2-0.5).
   Tests if spatial resource concentration creates de facto territories.
   Prediction: specialists near the rich zone dominate, creating natural specialization. */
...
__global__ void init_r_gradient(Resource*r,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    unsigned int s=(unsigned int)(i*2654435761u+99999);
    r[i].value=.5f+lcgf(&s)*.5f;r[i].collected=0;
    /* 70% in rich zone (0.2-0.5, 0.2-0.5) */
    if(i<(int)(n*0.7f)){
        r[i].x=.2f+lcgf(&s)*.3f;
        r[i].y=.2f+lcgf(&s)*.3f;
    }else{
...
__global__ void init_r_uniform(Resource*r,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    unsigned int s=(unsigned int)(i*2654435761u+99999);
    r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].value=.5f+lcgf(&s)*.5f;r[i].collected=0;
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v32 — Environmental Gradient (Rich Zone)\n");
    printf("  70%% resources in zone (0.2-0.5, 0.2-0.5)\n");
    printf("  A: gradient  B: uniform resources  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_grad=0,f_uni=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        init_a<<<blk,256>>>(da,NA);init_r_gradient<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_grad+=f;
        init_a<<<blk,256>>>(db,NA);init_r_uniform<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(db,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_uni+=f;
        init_c<<<blk,256>>>(dc,NA);init_r_gradient<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        ...
        printf("Exp %d: gradient=%.1f uniform=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_grad/=5;f_uni/=5;f_ctrl/=5;
    float r_g=(f_ctrl>.01f)?f_grad/f_ctrl:1,r_u=(f_ctrl>.01f)?f_uni/f_ctrl:1;
    float boost=(f_uni>.01f)?f_grad/f_uni:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Gradient: %.1f (%.2fx ctrl)\n",f_grad,r_g);
    printf("  Uniform:  %.1f (%.2fx ctrl)\n",f_uni,r_u);
    printf("  Control:  %.1f\n",f_ctrl);
    printf("  Gradient boost: %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"GRADIENT WINS":boost<0.9?"GRADIENT HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
