...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int grid_w, int mode, unsigned int seed, int *d_traces) {
    ...
    // Clear traces for this thread's simulation
    for(int i=0;i<GRID*GRID;i++) d_traces[tid * GRID * GRID + i] = 0;
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(!is_emitter && mode>0){
            ...
            if(mode==1){
                ...
                atomicAdd(&d_traces[tid * GRID * GRID + gy*grid_w+gx],1);
            }
            if(mode==2){
                // Historical traces
                atomicAdd(&d_traces[tid * GRID * GRID + gy*grid_w+gx],1);
            ...
            for(int dy2=-2;dy2<=2;dy2++)for(int dx2=-2;dx2<=2;dx2++){
                ...
                int v=d_traces[tid * GRID * GRID + ny2*grid_w+nx2];
...
int main(){
    ...
    float *d_s,*d_fx,*d_fy;int *d_a,*d_fa, *d_traces;
    ...
    cudaMalloc(&d_traces, N * GRID * GRID * sizeof(int));
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            cudaMemcpy(d_fx,hfx,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            cudaMemcpy(d_fy,hfy,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,GRID,m,(unsigned int)(42+tr*1111+m*111), d_traces);
    ...
    cudaFree(d_s);cudaFree(d_fx);cudaFree(d_fy);cudaFree(d_a);cudaFree(d_fa);cudaFree(d_traces);
    return 0;
}