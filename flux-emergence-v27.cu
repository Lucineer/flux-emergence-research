/* flux-emergence-v27.cu — Speed Asymmetry
   Novel: each archetype has a different movement speed multiplier.
   Explorers: 2x speed. Collectors: 1x. Communicators: 0.8x. Defenders: 0.5x.
   Tests if speed specialization matters more than uniform speed.
   Prediction: speed asymmetry amplifies role differences. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int asym){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    
    /* v27: speed multiplier by archetype */
    float spd=1.0f;
    if(asym){
        if(ag->arch==0)spd=2.0f;       /* explorers fast */
        else if(ag->arch==1)spd=1.0f;  /* collectors normal */
        else if(ag->arch==2)spd=0.8f;  /* communicators slow */
        else spd=0.5f;                 /* defenders very slow */
    }
    ...
    else if(br>=0){float dx=r[br].x-ag->x,dy=r[br].y-ag->y,d=sqrtf(dx*dx+dy*dy),sp=(.008f+cp*.008f+ep*.006f)*spd;ag->vx=ag->vx*.8f+(dx/d)*sp;ag->vy=ag->vy*.8f+(dy/d)*sp;}
    else{ag->vx=ag->vx*.95f+(lcgf(&ag->rng)-.5f)*.006f*(1+ep)*spd;ag->vy=ag->vy*.95f+(lcgf(&ag->rng)-.5f)*.006f*(1+ep)*spd;}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v27 — Speed Asymmetry\n");
    printf("  Exp:2x  Col:1x  Com:0.8x  Def:0.5x\n");
    printf("  A: asymmetric  B: uniform speed  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_as+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_un+=f;
        ...
        printf("Exp %d: asym=%.1f uniform=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_as/=5;f_un/=5;f_ctrl/=5;
    float r_as=(f_ctrl>.01f)?f_as/f_ctrl:1,r_un=(f_ctrl>.01f)?f_un/f_ctrl:1;
    float boost=(f_un>.01f)?f_as/f_un:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Asymmetric: %.1f (%.2fx ctrl)\n",f_as,r_as);
    printf("  Uniform:    %.1f (%.2fx ctrl)\n",f_un,r_un);
    printf("  Control:    %.1f\n",f_ctrl);
    printf("  Boost:      %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"ASYMMETRY WINS":boost<0.95?"HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
