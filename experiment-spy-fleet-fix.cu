...
__global__ void phase1_traces(int *traces, int steps, int n, int w, int grid_w, unsigned int seed) {
    ...
    for(int t=0;t<steps;t++){
        ...
        // Mix of steering and noise
        float sx=0,sy=0;
        float marks_x[10],marks_y[10];int nm=0;
        for(int m=0;m<nm;m++){
        ...
        dx+=sx;dy+=sy;
        dx+=(cr(&rng)-0.5f)*1.0f;dy+=(cr(&rng)-0.5f)*1.0f;
...
__global__ void phase2_exploit(float *scores, int *traces, float *fx, float *fy, int *falive,
    ...
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    unsigned int rng = seed + (tid+2*NF) * 997;
    float x = cr(&rng)*w, y = cr(&rng)*w, energy = 150.0f, score = 0.0f;
    float base_angle = (tid+2*NF) * 2.39996f;
    float dir[8];for(int i=0;i<8;i++)dir[i]=base_angle+i*0.785f;
...
int main(){
    const char* nm[]={"NoTraces","TracesA","TracesB","TracesA+B"};
    printf("=== SPY FLEET FIX: Law 264 ===\nFleet=128 Food=800 Steps=1500 Trials=%d\n\n",TRIALS);
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            cudaMemcpy(d_fx,hfx,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            cudaMemcpy(d_fy,hfy,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            // Phase 1: two fleets leave combined traces
            phase1_traces<<<blk,BLK>>>(d_tr,1500,NF,W,GRID,(unsigned int)(42+tr*1111));
            cudaDeviceSynchronize();
            // Phase 2: spy fleet
            phase2_exploit<<<blk,BLK>>>(d_s,d_tr,d_fx,d_fy,d_fa,1500,NF,FOOD,W,GRID,m,(unsigned int)(42+tr*1111));
            cudaDeviceSynchronize();
    ...
    printf("\n>> Law 264: Does more trace data (2 fleets) help spy fleet?\n");
