...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int use_steer, int use_scan, unsigned int seed) {
...
int main(){
    int modes_s[]={0,1,0,1};
    int modes_p[]={0,0,1,1};
    const char* nm[]={"PureScript","Steer","Scan","Steer+Scan"};
    printf("=== STEER vs SCAN: Law 231 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,modes_s[m],modes_p[m],(unsigned int)(42+tr*1111+m*111));
    ...
    printf("\n>> Law 231: Do steer and scan work through same or different mechanisms?\n");
