#include <stdio.h>
#include <stdlib.h>
#include <math.h>

#define N 1024
#define FOOD 400
#define STEPS 3000
#define W 256
#define BLK 128
#define NSPLITS 5
#define TRIALS 5

__device__ float cr(unsigned int *s) {
    *s ^= *s << 13; *s ^= *s >> 17; *s ^= *s << 5;
    return (*s & 0x7FFFFFFF) / (float)0x7FFFFFFF;
}

__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w,
    int explorer_frac, unsigned int seed) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    unsigned int rng = seed + tid * 997;
    float x = cr(&rng)*w, y = cr(&rng)*w, energy = 150.0f, score = 0.0f;
    float base_angle = tid * 2.39996f;
    float script_dir[8];
    for(int i=0;i<8;i++) script_dir[i]=base_angle+i*0.785f;
    int is_explorer = (tid < (n * explorer_frac / 100)) ? 1 : 0;
    float spd = is_explorer ? 4.0f : 1.0f;
    
    for(int t=0;t<steps&&energy>0;t++){
        int p=t%8;
        float dx=cosf(script_dir[p])*spd,dy=sinf(script_dir[p])*spd;
        float dist=sqrtf(dx*dx+dy*dy);
        energy-=0.005f+dist*0.003f;
        x=fmodf(x+dx+w,w);y=fmodf(y+dy+w,w);
        for(int i=0;i<food_count;i++){
            if(!falive[i])continue;
            float fdx=fx[i]-x,fdy=fy[i]-y;
            if(fdx>w/2)fdx-=w;if(fdx<-w/2)fdx+=w;
            if(fdy>w/2)fdy-=w;if(fdy<-w/2)fdy+=w;
            if(fdx*fdx+fdy*fdy<16.0f){int old=atomicExch(&falive[i],0);if(old){energy=fminf(energy+10.0f,200.0f);score+=1.0f;}}
        }
    }
    scores[tid]=score;alive[tid]=(energy>0)?1:0;
}

int main(){
    int splits[NSPLITS]={0,25,50,75,100};
    printf("=== SCAVENGER ECONOMY: Law 213 ===\n");
    printf("N=%d Food=%d Steps=%d Trials=%d\n",N,FOOD,STEPS,TRIALS);
    printf("Explorer=speed 4.0, Grazer=speed 1.0\n\n");
    float *d_s,*d_fx,*d_fy;int *d_a,*d_fa;
    cudaMalloc(&d_s,N*sizeof(float));cudaMalloc(&d_fx,FOOD*sizeof(float));
    cudaMalloc(&d_fy,FOOD*sizeof(float));cudaMalloc(&d_a,N*sizeof(int));cudaMalloc(&d_fa,FOOD*sizeof(int));
    float hfx[FOOD],hfy[FOOD];srand(42);
    for(int i=0;i<FOOD;i++){hfx[i]=((float)rand()/RAND_MAX)*W;hfy[i]=((float)rand()/RAND_MAX)*W;}
    cudaMemcpy(d_fx,hfx,FOOD*sizeof(float),cudaMemcpyHostToDevice);
    cudaMemcpy(d_fy,hfy,FOOD*sizeof(float),cudaMemcpyHostToDevice);
    int blk=(N+BLK-1)/BLK;
    
    printf("%-15s | %-8s | %-8s | %-10s | %-10s | %-10s\n",
        "Split","Score","Surv","FleetTotal","ExpScore","GrazScore");
    printf("----------------|----------|----------|------------|------------|----------\n");
    
    float best_fleet=0;int best_split=0;
    for(int s=0;s<NSPLITS;s++){
        float ts=0,ta=0,tt=0,te=0,tg=0;
        int exp_count=N*splits[s]/100;
        for(int tr=0;tr<TRIALS;tr++){
            cudaMemset(d_fa,1,FOOD*sizeof(int));
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,splits[s],(unsigned int)(42+tr*1111+s*111));
            cudaDeviceSynchronize();
            float hs[N];int ha[N];
            cudaMemcpy(hs,d_s,N*sizeof(float),cudaMemcpyDeviceToHost);
            cudaMemcpy(ha,d_a,N*sizeof(int),cudaMemcpyDeviceToHost);
            float avg=0;int ac=0;float total=0,se=0,sg=0,ce=0,cg=0;
            for(int i=0;i<N;i++){avg+=hs[i];ac+=ha[i];total+=hs[i];
                if(i<exp_count){se+=hs[i];ce++;}else{sg+=hs[i];cg++;}}
            ts+=avg/N;ta+=ac;tt+=total;te+=se/fmaxf(ce,1.0f);tg+=sg/fmaxf(cg,1.0f);
        }
        ts/=TRIALS;float surv=ta/TRIALS/N*100;tt/=TRIALS;te/=TRIALS;tg/=TRIALS;
        if(tt>best_fleet){best_fleet=tt;best_split=s;}
        printf("%3d%% exp %3d%% gr | %6.3f   | %5.1f%% | %8.1f   | %8.3f   | %6.3f\n",
            splits[s],100-splits[s],ts,surv,tt,te,tg);
    }
    printf("\nBest fleet: %d%% explorers (total=%.1f)\n",splits[best_split],best_fleet);
    printf(">> Law 213: Role specialization in scripted agents\n");
    cudaFree(d_s);cudaFree(d_fx);cudaFree(d_fy);cudaFree(d_a);cudaFree(d_fa);
    return 0;
}