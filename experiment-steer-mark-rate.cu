...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int mark_interval, unsigned int seed) {
    ...
    float marks_x[20], marks_y[20];int nmarks=0;
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(mark_interval>0 && t%mark_interval==0 && nmarks<20){marks_x[nmarks]=x;marks_y[nmarks]=y;nmarks++;}
...
int main(){
    int intervals[NRATES]={1,50,100,200,500,0};
    const char* nm[]={"Every1","Every50","Every100","Every200","Every500","NoMarks"};
    printf("=== MARK RATE: Law 233 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int r=0;r<NRATES;r++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,intervals[r],(unsigned int)(42+tr*1111+r*111));
        ...
        ts/=TRIALS;tt/=TRIALS;
        printf("%-12s: score=%.3f fleet=%.1f\n",nm[r],ts,tt);
    }
    printf("\n>> Law 233: Optimal mark placement frequency\n");
