/* flux-emergence-v37.cu — Dual-Layer Detection (Near + Far)
   Novel: agents have two detection zones — precise (grab range, high value) 
   and fuzzy (3x detection range, low value resources only). 
   This tests if layered sensing helps without memory.
   Prediction: dual-layer finds more resources but no fitness difference
   because the system is already detection-saturated. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int dual){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float det=.03f+ep*.04f,grab=.02f+cp*.02f;
    float det_far=(dual)?det*3:det; /* v37: far detection zone */
    float bd=det_far;int br=-1;
    ...
    if(br>=0&&bd<grab){
        ...
        /* v37: far targets get slower approach */
        if(dual&&bd>det)sp*=.5f; /* move slower to far targets */
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v37 — Dual-Layer Detection (Near + Far)\n");
    printf("  Far zone = 3x detection, slower approach\n");
    printf("  A: dual-layer  B: single  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_dual=0,f_single=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_dual+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_single+=f;
        ...
        printf("Exp %d: dual=%.1f single=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_dual/=5;f_single/=5;f_ctrl/=5;
    float boost=(f_single>.01f)?f_dual/f_single:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Dual:   %.1f (%.2fx ctrl)\n",f_dual,(f_ctrl>.01f)?f_dual/f_ctrl:1);
    printf("  Single: %.1f (%.2fx ctrl)\n",f_single,(f_ctrl>.01f)?f_single/f_ctrl:1);
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Boost:   %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"DUAL WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
