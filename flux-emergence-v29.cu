/* flux-emergence-v29.cu — Asymmetric Archetype Populations
   Novel: instead of equal 256 per archetype, try 50% collectors, 20% explorers,
   20% communicators, 10% defenders. More collectors = more resource gathering.
   Prediction: skewed populations boost total fitness by matching task demands. */
...
__device__ int assign_arch(int i,int skewed){
    if(!skewed)return i%N_ARCH;
    /* 50% collectors(1), 20% explorers(0), 20% communicators(2), 10% defenders(3) */
    int m=i%100;
    if(m<50)return 1;       /* collector */
    else if(m<70)return 0;  /* explorer */
    else if(m<90)return 2;  /* communicator */
    else return 3;          /* defender */
}

__global__ void init_a(Agent*a,int n,int skewed){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=assign_arch(i,skewed);a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v29 — Asymmetric Archetype Populations\n");
    printf("  50%% collectors, 20%% explorers, 20%% comms, 10%% defenders\n");
    printf("  A: skewed  B: balanced  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_sk=0,f_bal=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        init_a<<<blk,256>>>(da,NA,1);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_sk+=f;
        init_a<<<blk,256>>>(db,NA,0);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(db,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_bal+=f;
        ...
        /* Per-arch breakdown for skewed */
        float af[4]={0};for(int i=0;i<NA;i++)af[ha[i].arch]+=ha[i].fitness;
        printf("Exp %d: skew=%.1f bal=%.1f ctrl=%.1f | arch:",e+1,f/NA,f/NA,f/NA);
        for(int a=0;a<4;a++)printf(" %.0f",af[a]);
        printf("\n");
    }
    f_sk/=5;f_bal/=5;f_ctrl/=5;
    float r_sk=(f_ctrl>.01f)?f_sk/f_ctrl:1,r_bal=(f_ctrl>.01f)?f_bal/f_ctrl:1;
    float boost=(f_bal>.01f)?f_sk/f_bal:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Skewed:  %.1f (%.2fx ctrl)\n",f_sk,r_sk);
    printf("  Balanced: %.1f (%.2fx ctrl)\n",f_bal,r_bal);
    printf("  Control:  %.1f\n",f_ctrl);
    printf("  Skew boost: %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"SKEWED WINS":boost<0.95?"BALANCED WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
