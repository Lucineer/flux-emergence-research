...
__global__ void simulate(float *scores_a, float *scores_b, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int b_has_noise, unsigned int seed) {
    ...
    // Fleet B: Steer + Noise
    {
        ...
        for(int t=0;t<steps&&energy>0;t++){
            ...
            dx+=sx;dy+=sy;
            if(b_has_noise){dx+=(cr(&rng)-0.5f)*0.6f;dy+=(cr(&rng)-0.5f)*0.6f;}
            if(t%100==0&&nmarks<10){marks_x[nmarks]=x;marks_y[nmarks]=y;nmarks++;}
            float dist=sqrtf(dx*dx+dy*dy);energy-=0.005f+dist*0.003f;
            x=fmodf(x+dx+w,w);y=fmodf(y+dy+w,w);
            for(int i=0;i<food_count;i++){
...
int main(){
    ...
    for(int tr=0;tr<TRIALS;tr++){
        ...
        simulate<<<blk,BLK>>>(d_sa,d_sb,d_fx,d_fy,d_fa,STEPS,NF,FOOD,W,1,(unsigned int)(42+tr*1111));
    ...
    ta/=TRIALS;tb/=TRIALS;
    printf("Steer(A): %.3f  Steer+Noise(B): %.3f  ratio: %.2f\n",ta,tb,tb/(ta+0.001f));
    printf("\n>> Law 249: Does adding noise to steering help in competition?\n");
