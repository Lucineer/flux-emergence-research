...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float noise, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        dx+=(cr(&rng)-0.5f)*2.0f*noise;
        dy+=(cr(&rng)-0.5f)*2.0f*noise;
...
int main(){
    float noises[NNOISE]={0.0f,0.1f,0.3f,1.0f};
    const char* nm[]={"Pure","Noise0.1","Noise0.3","Noise1.0"};
    printf("=== SCRIPT NOISE: Law 244 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int i=0;i<NNOISE;i++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,noises[i],(unsigned int)(42+tr*1111+i*111));
            ...
            for(int j=0;j<N;j++){avg+=hs[j];ac+=ha[j];total+=hs[j];}
            ts+=avg/N;ta+=ac;tt+=total;
        }
        ts/=TRIALS;float surv=ta/TRIALS/N*100;tt/=TRIALS;
        if(i==0)base=ts;
        float lift=(base>0)?(ts-base)/base*100:0;
        printf("%-12s: score=%.3f surv=%.1f%% fleet=%.1f lift=%+.1f%%\n",nm[i],ts,surv,tt,lift);
    }
    printf("\n>> Law 244: Does random noise help or hurt scripted agents?\n");
