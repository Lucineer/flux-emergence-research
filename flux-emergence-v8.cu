/* flux-emergence-v8.cu — Scarcity + Territory + Message Passing.
   v7: 11% same as v6 — tips don't help when resources are abundant.
   v8: Make resources SCARCE (128 for 1024 agents), detection radius SMALL.
   Add territory: defenders near a resource boost collection for same-arch agents.
   Message passing should now matter: communicators share locations across territory.
   
   Hypothesis: with scarcity, information flow + territory = >30% advantage. */
...
__global__ void init_spec(Agent *a, int n) {
...
__global__ void init_ctrl(Agent *a, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    ...
    a[i].interactions = 0; a[i].group = -1;
    a[i].tip_x = a[i].tip_y = a[i].tip_val = 0.0f;
    for (int r = 0; r < 4; r++) a[i].role[r] = 0.25f;
}
...
__global__ void tick_spec(Agent *a, Resource *r, int na, int nr, int t, int pt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= na) return;
    Agent *ag = &a[i];
    float ep = ag->role[0], cp = ag->role[1], comm = ag->role[2], def = ag->role[3];

    /* v8: SMALL detection radius — scarcity matters */
    float detect = 0.03f + ep * 0.04f;   /* 0.03 to 0.07 */
    float grab = 0.02f + cp * 0.02f;     /* 0.02 to 0.04 */
    ...
    /* Follow tip if no resource detected */
    if (br < 0 && ag->tip_val > 0.3f) {
        ...
        for (int j = 0; j < nr; j++) {
            ...
            if (sqrtf(dx*dx+dy*dy) < 0.03f) {
                float full = td + 0.03f;
                if (full < detect * 2.0f) { bd = full; br = j; }
                break;
            }
        }
        ag->tip_val *= 0.95f;
    ...
    if (br >= 0 && bd < grab) {
        ...
        /* Territory bonus: defenders nearby boost collection */
        float terr_bonus = 1.0f;
        for (int k = 0; k < 16; k++) {
            int j = lcg(&ag->rng) % na;
            if (j == i || a[j].arch != ag->arch) continue;
            float dx = a[j].x - ag->x, dy = a[j].y - ag->y;
            if (sqrtf(dx*dx+dy*dy) < 0.05f) {
                terr_bonus += a[j].role[3] * 0.2f; /* defender boosts collector */
            }
        }
        float bonus = (1.0f + cp * 0.5f) * terr_bonus;
        ag->energy = fminf(1.0f, ag->energy + r[br].value * 0.1f * bonus);
        ag->fitness += r[br].value * bonus;
    } else if (br >= 0) {
        float tx = r[br].x, ty = r[br].y;
    ...
    for (int k = 0; k < 32; k++) {
        ...
        if (a[j].role[2] > 0.5f && comm > 0.2f) {
            ...
            if (jbr >= 0) {
                ag->tip_x = r[jbr].x; ag->tip_y = r[jbr].y;
                ag->tip_val = a[j].role[2]; /* stronger comm = better tips */
            }
        }
...
__global__ void tick_ctrl(Agent *a, Resource *r, int na, int nr, int t, int pt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= na) return;
    Agent *ag = &a[i];
    float detect = 0.05f; float grab = 0.03f;
    float bd = detect; int br = -1;
    for (int j = 0; j < nr; j++) {
        if (r[j].collected) continue;
        float dx = r[j].x - ag->x, dy = r[j].y - ag->y;
    ...
    if (br >= 0 && bd < grab) {
        ...
        float d = sqrtf(dx*dx + dy*dy);
        ag->vx = ag->vx * 0.8f + (dx/d) * 0.014f;
        ag->vy = ag->vy * 0.8f + (dy/d) * 0.014f;
    } else {
        ag->vx = ag->vx * 0.95f + (lcgf(&ag->rng) - 0.5f) * 0.008f;
        ag->vy = ag->vy * 0.95f + (lcgf(&ag->rng) - 0.5f) * 0.008f;
    }
    ag->x = fmodf(ag->x + ag->vx + 1.0f, 1.0f);
    ag->y = fmodf(ag->y + ag->vy + 1.0f, 1.0f);
    for (int k = 0; k < 32; k++) {
...
int main() {
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v8 — Scarcity + Territory + Comms\n");
    printf("  1024 agents, 128 resources, small detection radius\n");
    printf("  Territory: defenders boost collectors\n");
    printf("  Comms: communicators share locations\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<N_EXP;e++){
        init_spec<<<blk,256>>>(da,N_AGENTS);
        init_res<<<rblk,256>>>(dr,N_RESOURCES);
        cudaDeviceSynchronize();
        for(int t=0;t<MAX_TICKS;t++){
            tick_spec<<<blk,256>>>(da,dr,N_AGENTS,N_RESOURCES,t,MAX_TICKS/2);
            cudaDeviceSynchronize();
        }
        ...
        init_ctrl<<<blk,256>>>(db,N_AGENTS);
        init_res<<<rblk,256>>>(dr,N_RESOURCES);
        cudaDeviceSynchronize();
        for(int t=0;t<MAX_TICKS;t++){
            tick_ctrl<<<blk,256>>>(db,dr,N_AGENTS,N_RESOURCES,t,MAX_TICKS/2);
            cudaDeviceSynchronize();
        }
    ...
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  v6-v7 (abundant): 11%%\n");
    printf("  v8 (scarce+territory+comms): %.0f%%\n", (ratio_avg-1.0f)*100.0f);
    printf("  Clustering:     %.3f\n",as_sil);
    printf("  Specialization: %.3f\n",as_spec);
    printf("  A/B ratio:      %.2fx %s\n",ratio_avg,
        ratio_avg>1.5?"EMERGENT":ratio_avg>1.3?"SIGNIFICANT":ratio_avg>1.15?"MODERATE":"MARGINAL");
