...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float speed, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        int p=t%8;float dx=cosf(dir[p])*2.0f*spd,dy=sinf(dir[p])*2.0f*spd;
...
__global__ void simulate_mixed(float *scores, int *alive, float *fx, float *fy, int *falive,
...
int main(){
    const char* nm[]={"All-1.0","All-2.0","Mixed(1+3)"};
    printf("=== SPEED DIVERSITY: Law 248 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            if(m<2) simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,m==0?1.0f:2.0f,(unsigned int)(42+tr*1111+m*111));
            else simulate_mixed<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,(unsigned int)(42+tr*1111+m*111));
        ...
        printf("%-14s: score=%.3f surv=%.1f%% fleet=%.1f\n",nm[m],ts,surv,tt);
    }
    printf("\n>> Law 248: Does speed diversity help?\n");
