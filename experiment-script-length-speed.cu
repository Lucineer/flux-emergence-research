...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    float speed_mult, int script_len, int steps, int n, int food_count, int w, unsigned int seed) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    unsigned int rng = seed + tid * 997;
    float x = cr(&rng)*w, y = cr(&rng)*w, energy = 150.0f, score = 0.0f;
    float base_angle = tid * 2.39996f;
    // Use a fixed-size array with maximum script length (128)
    float dir[128];
    for(int i=0;i<script_len;i++)dir[i]=base_angle+i*6.2832f/script_len;
    for(int t=0;t<steps&&energy>0;t++){
        int p=t%script_len;
        float dx=cosf(dir[p])*2.0f*speed_mult,dy=sinf(dir[p])*2.0f*speed_mult;
...
int main(){
    int lengths[NLENGTHS]={8,16,32,64,128};
    float speeds[NSPEEDS]={1.0f,4.0f,16.0f};
    printf("=== SCRIPT LENGTH vs SPEED: Law 236 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int sp=0;sp<NSPEEDS;sp++){
        ...
        for(int l=0;l<NLENGTHS;l++){
            ...
            for(int tr=0;tr<TRIALS;tr++){
                ...
                simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,speeds[sp],lengths[l],STEPS,N,FOOD,W,(unsigned int)(42+tr*1111+sp*111+l*11));
            ...
            ts/=TRIALS;tt/=TRIALS;
            if(ts>best){best=ts;bl=l;}
            printf("  Length %-3d: score=%.3f fleet=%.1f\n",lengths[l],ts,tt);
        }
        printf("  Best: length %d (score=%.3f)\n\n",lengths[bl],best);
    }
