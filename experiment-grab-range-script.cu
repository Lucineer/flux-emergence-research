...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float grab_r, unsigned int seed) {
...
int main(){
    float ranges[NRANGES]={2.0f,4.0f,6.0f,8.0f,12.0f,16.0f};
    printf("=== GRAB RANGE FOR SCRIPTED: Law 222 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int r=0;r<NRANGES;r++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,ranges[r],(unsigned int)(42+tr*1111+r*111));
        ...
        if(r==0)baseline=ts;
        float lift=(baseline>0)?(ts-baseline)/baseline*100:0;
        printf("%-8.0f | %6.3f   | %5.1f%% | %8.1f   (%+.0f%%)\n",ranges[r],ts,surv,tt,lift);
    }
    printf("\n>> Law 222: Grab range effect on scripted agents\n");
