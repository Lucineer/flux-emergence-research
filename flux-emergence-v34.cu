/* flux-emergence-v34.cu — Collective Voting (Swarm Intelligence)
   Novel: each agent samples 8 neighbors and moves toward the majority direction.
   Tests if collective decision-making improves resource finding over individual.
   Prediction: voting helps explorers find resources faster. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int voting){
    ...
    if(br>=0&&bd<grab){r[br].collected=1;ag->res_held++;float tb=1;for(int k=0;k<16;k++){int j=lcg(&ag->rng)%na;if(j==i||a[j].arch!=ag->arch)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y;if(sqrtf(dx*dx+dy*dy)<.05f)tb+=a[j].role[3]*.2f;}float bn=(1+cp*.5f)*tb;ag->energy=fminf(1,ag->energy+r[br].value*.1f*bn);ag->fitness+=r[br].value*bn;}
        ...
        /* v34: collective voting — sample neighbors, move toward majority direction */
        if(voting){
            float vote_x=0,vote_y=0;
            for(int k=0;k<8;k++){
                int j=lcg(&ag->rng)%na;if(j==i)continue;
                float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);
                if(dist<.08f&&dist>.001f){vote_x+=a[j].vx;vote_y+=a[j].vy;}
            }
            float vm=sqrtf(vote_x*vote_x+vote_y*vote_y);
            if(vm>.001f){
                float w=ep*.003f; /* explorers follow swarm more */
                ag->vx+=vote_x/vm*w;
                ag->vy+=vote_y/vm*w;
            }
        }
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v34 — Collective Voting (Swarm Intelligence)\n");
    printf("  Agents follow majority movement of neighbors\n");
    printf("  A: voting  B: individual  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_vot=0,f_ind=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_vot+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_ind+=f;
        ...
        printf("Exp %d: voting=%.1f indiv=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_vot/=5;f_ind/=5;f_ctrl/=5;
    float boost=(f_ind>.01f)?f_vot/f_ind:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Voting:  %.1f (%.2fx ctrl)\n",f_vot,(f_ctrl>.01f)?f_vot/f_ctrl:1);
    printf("  Indiv:   %.1f (%.2fx ctrl)\n",f_ind,(f_ctrl>.01f)?f_ind/f_ctrl:1);
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Boost:   %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"VOTING WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
