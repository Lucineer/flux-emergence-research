/* flux-emergence-v30.cu — Reciprocal Altruism (Memory-based)
   Novel: each agent remembers who gave it tips. When it finds resources,
   it preferentially tips back those who helped it.
   Tests if reciprocity improves information flow beyond random comms.
   Prediction: reciprocal networks form, improving tip quality by 10%. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,group;float tip_x,tip_y,tip_val;int helpers[N_MEMORY];int n_helpers;unsigned int rng;}Agent;
typedef struct{float x,y,value;int collected;}Resource;

__global__ void init_a(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].n_helpers=0;for(int m=0;m<N_MEMORY;m++)a[i].helpers[m]=-1;for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}

__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].n_helpers=0;for(int m=0;m<N_MEMORY;m++)a[i].helpers[m]=-1;for(int r=0;r<4;r++)a[i].role[r]=.25f;}
...
__device__ void add_helper(Agent*ag,int j){
    for(int m=0;m<ag->n_helpers;m++)if(ag->helpers[m]==j)return;
    if(ag->n_helpers<N_MEMORY)ag->helpers[ag->n_helpers++]=j;
    else{ag->helpers[ag->n_helpers%N_MEMORY]=j;ag->n_helpers++;}
}

__device__ int is_helper(Agent*ag,int j){for(int m=0;m<N_MEMORY;m++)if(ag->helpers[m]==j)return 1;return 0;}

__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int reciprocal){
    ...
    if(br>=0&&bd<grab){
        ...
        /* v30: when collecting, preferentially tip helpers (reciprocal) */
        if(reciprocal){
            int tipped=0;
            for(int m=0;m<N_MEMORY&&m<ag->n_helpers;m++){
                int hj=ag->helpers[m];if(hj<0||hj>=na)continue;
                float dx=a[hj].x-ag->x,dy=a[hj].y-ag->y,dist=sqrtf(dx*dx+dy*dy);
                if(dist<.08f){a[hj].tip_x=r[br].x;a[hj].tip_y=r[br].y;a[hj].tip_val=ag->role[2];tipped++;}
            }
        }
    ...
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);if(dist>=.06f)continue;ag->interactions++;
        ...
        if(a[j].role[2]>.5f&&cm>.2f){float jbd=.1f;int jbr=-1;for(int m=0;m<nr;m++){if(r[m].collected)continue;float mdx=r[m].x-a[j].x,mdy=r[m].y-a[j].y,md=sqrtf(mdx*mdx+mdy*mdy);if(md<jbd){jbd=md;jbr=m;}}if(jbr>=0){ag->tip_x=r[jbr].x;ag->tip_y=r[jbr].y;ag->tip_val=a[j].role[2];
            /* v30: remember who helped you */
            if(reciprocal)add_helper(ag,j);
        }}
    ...
    if(t==pt){ag->energy*=(1-.5f*(1-df*.5f));ag->x=lcgf(&ag->rng);ag->y=lcgf(&ag->rng);ag->vx=ag->vy=0;ag->tip_val=0;ag->n_helpers=0;}
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v30 — Reciprocal Altruism\n");
    printf("  Agents remember helpers, preferentially tip them back\n");
    printf("  A: reciprocal  B: random tips  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_rec=0,f_rnd=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_rec+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_rnd+=f;
        ...
        printf("Exp %d: recip=%.1f random=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_rec/=5;f_rnd/=5;f_ctrl/=5;
    float r_rec=(f_ctrl>.01f)?f_rec/f_ctrl:1,r_rnd=(f_ctrl>.01f)?f_rnd/f_ctrl:1;
    float boost=(f_rnd>.01f)?f_rec/f_rnd:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Reciprocal: %.1f (%.2fx ctrl)\n",f_rec,r_rec);
    printf("  Random:     %.1f (%.2fx ctrl)\n",f_rnd,r_rnd);
    printf("  Control:    %.1f\n",f_ctrl);
    printf("  Boost:      %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.05?"RECIPROCITY WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
