...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float bias_x, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        // Food drifts east at 0.5 units/step
        if(tid==0){
            for(int i=0;i<food_count;i++){
                fx[i]=fmodf(fx[i]+0.5f+w,w);
            }
        }
        __syncthreads();
        
        int p=t%8;float dx=cosf(dir[p])*2.0f+bias_x,dy=sinf(dir[p])*2.0f;
        ...
        for(int i=0;i<food_count;i++){
            ...
            if(fdx*fdx+fdy*fdy<16.0f){
                ...
                if(old){
                    energy=fminf(energy+10.0f,200.0f);
                    score+=1.0f;
                }
            }
        }
...
int main(){
    const char* nm[]={"Normal","East-bias(+0.5)","West-bias(-0.5)"};
    float biases[]={0.0f,0.5f,-0.5f};
    printf("=== RESOURCE MIGRATION: Law 256 ===\nN=%d Food=%d Steps=%d Trials=%d\n",N,FOOD,STEPS,TRIALS);
    printf("Food drifts east at 0.5 units/step\n\n");
    ...
    for(int m=0;m<NMODES;m++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,biases[m],(unsigned int)(42+tr*1111+m*111));
        ...
        printf("%-20s: score=%.3f surv=%.1f%% fleet=%.1f\n",nm[m],ts,surv,tt);
    }
    printf("\n>> Law 256: Does matching resource flow direction help?\n");
