/* experiment-coop-threshold.cu — How many agents needed for cooperative collection?
   v40 used threshold=2. Sweep 2, 3, 4, 5, 8.
   Prediction: higher thresholds = harder to reach = less benefit, but more value per collection. */
...
__global__ void init_r(Resource*r,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].collected=0;r[i].heavy=(i<(int)(n*0.3f))?1:0;r[i].value=r[i].heavy?1.5f:(.5f+lcgf(&s)*.5f);}
...
__global__ void tick_s(Agent*a,Resource*r,int*nc,int na,int nr,int t,int pt,int thresh){
    ...
    if(br>=0&&bd<grab){
        if(r[br].heavy&&nc[br]<thresh){} // can't collect — not enough agents
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  Experiment: Cooperative Threshold Sweep\n");
    printf("  How many agents needed to collect heavy resources?\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    f_ctrl/=5;
    printf("Control: %.1f\n\n",f_ctrl);
    int thresholds[]={1,2,3,4,5,8};
    for(int ti=0;ti<6;ti++){
        float f=0;
        for(int e=0;e<5;e++){
            init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
            for(int t=0;t<MAXT;t++){count_near<<<rblk,256>>>(dr,da,NR,NA,nc);tick_s<<<blk,256>>>(da,dr,nc,NA,NR,t,250,thresholds[ti]);cudaDeviceSynchronize();}
        ...
        printf("  Threshold %d: %.1f (%.2fx ctrl)\n",thresholds[ti],f,ratio);
    }
    printf("\n═══════════════════════════════════════════════════════\n");
