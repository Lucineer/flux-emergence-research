...
__global__ void ss(int step,int use_dcs){
    ...
    if(use_dcs&&dcs_v[0]){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            bd=999999;bf=-1;
            for(int f=0;f<FOOD;f++){if(!falive[f])continue;int d=td(ax[i],ay[i],fx[f],fy[f]);if(d<bd){bd=d;bf=f;}}
            if(dd<=g2&&bf>=0&&falive[bf]){falive[bf]=0;acol[i]++;grab_success[0]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
            else if(bf>=0&&bd<=g2&&falive[bf]){falive[bf]=0;acol[i]++;grab_success[0]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
            else grab_wasted[0]++;
            return;
        }
    }
    if(bf>=0&&bd<=g2){
        if(falive[bf]){falive[bf]=0;acol[i]++;grab_success[0]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
        else grab_wasted[0]++;
    }else if(bf>=0){
...
int main(){
    ...
    printf("=== DCS Effect on Waste Rate ===\\n");
    ...
    for(int mode=0;mode<2;mode++){
        ...
        int dz[1];dz[0]=0;cudaMemcpyToSymbol(dcs_v,dz,sizeof(int));
        ...
        printf("%s: %.0f/agent, waste=%.1f%% (%d wasted, %d success)\\n",mode?"DCS":"NoDCS",(float)t/NA,100.0*gw/(gw+gs),gw,gs);
    }
    return 0;
}
