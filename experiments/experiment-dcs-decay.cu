...
__device__ int dcs_x[1],dcs_y[1],dcs_v[1],dcs_age[1];
__device__ int decay_steps; // DCS ignored if older than this
...
__global__ void ss(int step){
    ...
    int fresh=dcs_v[0]&&(decay_steps==0||dcs_age[0]<decay_steps);
    if(fresh){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            bd=999999;bf=-1;
            for(int f=0;f<FOOD;f++){if(!falive[f])continue;int d=td(ax[i],ay[i],fx[f],fy[f]);if(d<bd){bd=d;bf=f;}}
            if(dd<=g2&&bf>=0&&falive[bf]){falive[bf]=0;acol[i]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;dcs_age[0]=0;}
            else if(bf>=0&&bd<=g2&&falive[bf]){falive[bf]=0;acol[i]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;dcs_age[0]=0;}
            return;
        }
    }
    if(bf>=0&&bd<=g2){if(falive[bf]){falive[bf]=0;acol[i]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;dcs_age[0]=0;}}
...
int main(){
    ...
    printf("=== DCS Stale Decay ===\\n");
    printf("4096 agents, 400 food, grab=12, 3000 steps\\n");
    printf("MaxAge | Collection | vs None\\n");
    printf("-----------------------------\\n");
    int ages[]={0,1,2,5,10,50,100,500,0};
    float pa[9];
    for(int ai=0;ai<9;ai++){
        pa[ai]=0;
        for(int trial=0;trial<2;trial++){
            int z[2];z[0]=0;z[1]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));cudaMemcpyToSymbol(dcs_age,z+1,sizeof(int));
            cudaMemcpyToSymbol(decay_steps,&ages[ai],sizeof(int));
            ...
            long t=0;for(int i=0;i<NA;i++)t+=hc[i];pa[ai]+=(float)t/NA;
        }
        pa[ai]/=2;
    }
    char*labels[]={"NoDCS","1","2","5","10","50","100","500","Inf"};
    for(int a=0;a<9;a++){
        printf("  %4s | %9.1f |",labels[a],pa[a]);
        if(a==0)printf(" baseline\\n");else printf(" %.2fx\\n",pa[a]/pa[0]);
    }
    return 0;
}
