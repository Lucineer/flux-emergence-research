...
__global__ void init_f_scatter(int seed){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=FOOD)return;
    int s=seed+i*777;fx[i]=s%SZ;fy[i]=(s*31)%SZ;falive[i]=1;ftimer[i]=0;
}
__global__ void init_f_cluster(int seed,int clusters,int radius){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=FOOD)return;
    int ci=i%clusters;
    int cx=(seed+ci*137)%SZ,cy=(seed+ci*251)%SZ;
    fx[i]=(cx+rn(&seed+i)%radius+SZ)%SZ;
    fy[i]=(cy+rn(&seed+i)%radius+SZ)%SZ;
...
int main(){
    ...
    printf("=== Food Clustering x DCS ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    printf("Layout     | NoDCS | DCS | Lift\\n");
    printf("------------------------------\\n");
    // 0=scatter, 1=4 clusters r=20, 2=4 clusters r=5, 3=1 cluster r=30, 4=1 cluster r=10
    int modes[]={0,1,2,3,4};
    char*names[]={"Scatter","4-clust-r20","4-clust-r5","1-clust-r30","1-clust-r10"};
    for(int mi=0;mi<5;mi++){
        float pa_nodcs=0,pa_dcs=0;
        for(int trial=0;trial<3;trial++){
            int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<32,BLK>>>(42+trial);
            if(modes[mi]==0)init_f_scatter<<<fb,BLK>>>(999+trial);
            else if(modes[mi]==1)init_f_cluster<<<fb,BLK>>>(999+trial,4,20);
            else if(modes[mi]==2)init_f_cluster<<<fb,BLK>>>(999+trial,4,5);
            else if(modes[mi]==3)init_f_cluster<<<fb,BLK>>>(999+trial,1,30);
            else init_f_cluster<<<fb,BLK>>>(999+trial,1,10);
        ...
        pa_nodcs/=3;pa_dcs/=3;
        printf("%-12s| %5.0f | %3.0f | %.2fx\\n",names[mi],pa_nodcs,pa_dcs,pa_dcs/pa_nodcs);
    }
    return 0;
}
