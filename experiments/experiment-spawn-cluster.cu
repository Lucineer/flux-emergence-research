...
__global__ void init_w_random(int seed){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;
    aseed[i]=seed+i*137;ax[i]=rn(&aseed[i])%SZ;ay[i]=rn(&aseed[i])%SZ;acol[i]=0;
}
__global__ void init_w_cluster(int seed){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;
    aseed[i]=seed+i*137;
    // All agents spawn near center with small jitter
    int cx=rn(&aseed[i])%SZ; // same center for all (race condition but ok)
    ax[i]=(cx+rn(&aseed[i])%16-8+SZ)%SZ;
    ay[i]=(cx+rn(&aseed[i])%16-8+SZ)%SZ; // cluster in one spot
    acol[i]=0;
}
...
int main(){
    ...
    printf("=== Spawn Clustering x DCS ===\\n");
    printf("Spawn   | NoDCS | DCS | DCS Lift\\n");
    printf("------------------------------\\n");
    // Test random spawn
    float pa_rnd_nodcs=0,pa_rnd_dcs=0,pa_cls_nodcs=0,pa_cls_dcs=0;
    for(int trial=0;trial<3;trial++){
        int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
        init_w_random<<<32,BLK>>>(42+trial);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
        ...
        long t=0;for(int i=0;i<NA;i++)t+=hc[i];pa_rnd_nodcs+=(float)t/NA;
        ...
        t=0;for(int i=0;i<NA;i++)t+=hc[i];pa_rnd_dcs+=(float)t/NA;
        z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
        init_w_cluster<<<32,BLK>>>(42+trial);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
        for(int s=0;s<STEPS;s++){ss<<<32,BLK>>>(s,0);do_resp<<<fb,BLK>>>(0);if(s%500==0)cudaDeviceSynchronize();}
        cudaDeviceSynchronize();cudaMemcpyFromSymbol(hc,acol,sizeof(int)*NA);
        t=0;for(int i=0;i<NA;i++)t+=hc[i];pa_cls_nodcs+=(float)t/NA;
        z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
        init_w_cluster<<<32,BLK>>>(42+trial);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
        for(int s=0;s<STEPS;s++){ss<<<32,BLK>>>(s,1);do_resp<<<fb,BLK>>>(0);if(s%500==0)cudaDeviceSynchronize();}
        cudaDeviceSynchronize();cudaMemcpyFromSymbol(hc,acol,sizeof(int)*NA);
        t=0;for(int i=0;i<NA;i++)t+=hc[i];pa_cls_dcs+=(float)t/NA;
    }
    pa_rnd_nodcs/=3;pa_rnd_dcs/=3;pa_cls_nodcs/=3;pa_cls_dcs/=3;
    printf("Random | %5.0f | %3.0f | %.2fx\\n",pa_rnd_nodcs,pa_rnd_dcs,pa_rnd_dcs/pa_rnd_nodcs);
    printf("Cluster| %5.0f | %3.0f | %.2fx\\n",pa_cls_nodcs,pa_cls_dcs,pa_cls_dcs/pa_cls_nodcs);
    printf("\\nCluster advantage: %.2fx (NoDCS), %.2fx (DCS)\\n",pa_cls_nodcs/pa_rnd_nodcs,pa_cls_dcs/pa_rnd_dcs);
    return 0;
}
