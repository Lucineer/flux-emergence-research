...
__device__ int fx[800],fy[800],falive[800];
...
__global__ void migrate_food(int food,int speed,int mode){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=food)return;
    if(!falive[i])return;
    if(mode==0){
        fx[i]=to(fx[i]+speed);
...
int main(){
    int food=200;int fb=(food+BLK-1)/BLK;
    printf("=== DCS + Migrating Food ===\n");
    printf("4096 agents, 200 food, 3000 steps\n");
    printf("Mode | Speed | NoDCS/agent | DCS/agent | Lift\n");
    printf("------------------------------------------\n");
    int modes[]={0,1,2};char*mn[]={"linear-X","linear-Y","diagonal"};
    int speeds[]={0,1,2,4};
    for(int mi=0;mi<3;mi++){
        for(int si=0;si<4;si++){
            ...
            for(int trial=0;trial<3;trial++){
                ...
                for(int s=0;s<STEPS;s++){ss<<<32,BLK>>>(food,s,0);migrate_food<<<fb,BLK>>>(food,speeds[si],modes[mi]);do_resp<<<fb,BLK>>>(food);if(s%500==0)cudaDeviceSynchronize();}
                ...
                cudaMemcpyToSymbol(dcs_v,z,sizeof(int)*GUILDS);
                init_w<<<32,BLK>>>(42+trial);init_f<<<fb,BLK>>>(food,999+trial);cudaDeviceSynchronize();
                for(int s=0;s<STEPS;s++){ss<<<32,BLK>>>(food,s,1);migrate_food<<<fb,BLK>>>(food,speeds[si],modes[mi]);do_resp<<<fb,BLK>>>(food);if(s%500==0)cudaDeviceSynchronize();}
                cudaDeviceSynchronize();
                cudaMemcpyFromSymbol(hc,acol,sizeof(int)*NA);
                t=0;for(int i=0;i<NA;i++)t+=hc[i];pa_dcs+=(float)t/NA;
            }
            pa_nodcs/=3;pa_dcs/=3;
            printf("%-10s | %d | %11.1f | %9.1f | %.2fx\n",mn[mi],speeds[si],pa_nodcs,pa_dcs,pa_dcs/pa_nodcs);
        }
    }
    return 0;
}
