...
__global__ void init_w(int seed){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;aseed[i]=seed+i*137;ax[i]=rn(&aseed[i])%SZ;ay[i]=rn(&aseed[i])%SZ;acol[i]=0;aspeed[i]=1;}
...
__global__ void ss(int step,int mode){
    ...
    // Learning: adjust speed based on success
    if(mode>=1){
        if(collected&&spd<4)aspeed[i]=spd+1; // speed up on success
        if(!collected&&!moved&&step%50==0&&spd>1)aspeed[i]=spd-1; // slow down when stuck
    }
    // Mode 2: random speed perturbation
    if(mode==2&&step%100==0){aspeed[i]=1+rn(&aseed[i])%3;}
}
int main(){
    int fb=(FOOD+BLK-1)/BLK;
    printf("=== Adaptive Speed Learning ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    float pa[3];
    char*names[]={"Fixed-1","Learn","Random"};
    for(int mode=0;mode<3;mode++){
        ...
        for(int trial=0;trial<3;trial++){
            ...
            int hc[NA],hs[NA];cudaMemcpyFromSymbol(hc,acol,sizeof(int)*NA);cudaMemcpyFromSymbol(hs,aspeed,sizeof(int)*NA);
            long t=0;for(int i=0;i<NA;i++)t+=hc[i];pa[mode]+=(float)t/NA;
        }
        pa[mode]/=3;
    }
    for(int m=0;m<3;m++){
        if(m==0)printf("  %-12s | %.0f/agent (baseline)\\n",names[m],pa[m]);
        else printf("  %-12s | %.0f/agent (%.2fx)\\n",names[m],pa[m],pa[m]/pa[0]);
    }
    return 0;
}
