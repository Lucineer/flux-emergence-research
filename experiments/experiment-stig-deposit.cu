...
__global__ void ss(int step){
    ...
    if(bf>=0&&bd<=g2){
        if(falive[bf]){falive[bf]=0;acol[i]++;heat[to(fy[bf])*SZ+to(fx[bf])]+=deposit_amt;}
    }else if(bf>=0){
...
int main(){
    ...
    printf("=== Stigmergy Deposit Amount ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    printf("Deposit | Collection | vs None\\n");
    printf("----------------------------------\\n");
    int deps[]={0,1,5,10,20,50,100};
    float none_pa=0;
    for(int di=0;di<7;di++){
        cudaMemcpyToSymbol(deposit_amt,&deps[di],sizeof(int));
        float pa=0;
        ...
        pa/=3;
        if(di==0)none_pa=pa;
        printf("  %4d  | %9.1f |",deps[di],pa);
        if(di==0)printf(" baseline\\n");else printf(" %.2fx\\n",pa/none_pa);
    }
    return 0;
}
