...
#define BLK 128
#define STEPS 2000
__device__ int ax[NA],ay[NA],acol[NA],aseed[NA],agrab[NA];
...
__global__ void init_w(int seed,int mode){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;aseed[i]=seed+i*137;ax[i]=rn(&aseed[i])%SZ;ay[i]=rn(&aseed[i])%SZ;acol[i]=0;if(mode==0)agrab[i]=12;else if(mode==1)agrab[i]=6+(i%3)*6;else agrab[i]=4+(rn(&aseed[i])%20);}
...
__global__ void ss(int step,int use_dcs){
    ...
    int g2=agrab[i]*agrab[i],bd=999999,bf=-1;
...
int main(){
    ...
    printf("=== Heterogeneous Grab Ranges ===\\n");
    printf("4096 agents, 400 food, 2000 steps\\n");
    printf("Config | NoDCS | DCS | DCS lift\\n");
    printf("------------------------------\\n");
    for(int grab_mode=0;grab_mode<3;grab_mode++){
        float pn=0,pd=0;
        for(int trial=0;trial<2;trial++){
            int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<32,BLK>>>(42+trial,grab_mode);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
        ...
        pn/=2;pd/=2;
        char*names[]={"Uniform-12","Tiered-6/12/18","Random-4-23"};
        printf("  %-14s | %5.0f | %4.0f | %.2fx\\n",names[grab_mode],pn,pd,pd/pn);
    }
    return 0;
}
