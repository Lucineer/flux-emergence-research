...
int main(){
    ...
    printf("=== Grab Range x DCS Interaction ===\n");
    printf("4096 agents, 400 food, 3000 steps\n");
    printf("Grab | NoDCS/agent | DCS/agent | DCS Lift\n");
    printf("------------------------------------------\n");
    float grabs[]={4.0f,6.0f,8.0f,12.0f,16.0f,24.0f,32.0f};
    for(int gi=0;gi<7;gi++){
        cudaMemcpyToSymbol(gr_val,&grabs[gi],sizeof(float));
        ...
        pa_nodcs/=3;pa_dcs/=3;
        printf(" %4.0f | %11.1f | %9.1f | %7.2fx\n",grabs[gi],pa_nodcs,pa_dcs,pa_dcs/pa_nodcs);
    }
    return 0;
}
