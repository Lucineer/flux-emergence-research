...
__device__ int ax[MAXNA],ay[MAXNA],acol[MAXNA],aseed[MAXNA];
...
__global__ void init_w(int seed,int na){
...
__global__ void ss(int step,int na,int use_dcs){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    int g2=144;
    int bd=999999,bf=-1;
    ...
    if(bf>=0&&bd<=g2){
        ...
        if(dx<-SZ/2)dx+=SZ;if(dx>SZ/2)dx-=SZ;if(dy<-SZ/2)dy+=SZ;if(dy>SZ/2)dy-=SZ;
        if(dx!=0||dy!=0){ax[i]=to(ax[i]+dx);ay[i]=to(ax[i]+dy);}
    }
}
int main(){
    int fb=(FOOD+BLK-1)/BLK;
    printf("=== Population x DCS ===\\n");
    printf("Pop   | NoDCS/agent | DCS/agent | Lift\\n");
    printf("---------------------------------------\\n");
    int pops[]={128,256,512,1024,2048,4096};
    for(int pi=0;pi<6;pi++){
        int na=pops[pi];int nb=(na+BLK-1)/BLK;
        float pa_nodcs=0,pa_dcs=0;
        for(int trial=0;trial<3;trial++){
            int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<nb,BLK>>>(42+trial,na);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
            for(int s=0;s<STEPS;s++){ss<<<nb,BLK>>>(s,na,0);do_resp<<<fb,BLK>>>(0);if(s%500==0)cudaDeviceSynchronize();}
            cudaDeviceSynchronize();
            int hc[MAXNA];cudaMemcpyFromSymbol(hc,acol,sizeof(int)*MAXNA);
            long t=0;for(int i=0;i<na;i++)t+=hc[i];pa_nodcs+=(float)t/na;
            z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<nb,BLK>>>(42+trial,na);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
            for(int s=0;s<STEPS;s++){ss<<<nb,BLK>>>(s,na,1);do_resp<<<fb,BLK>>>(0);if(s%500==0)cudaDeviceSynchronize();}
            cudaDeviceSynchronize();
            cudaMemcpyFromSymbol(hc,acol,sizeof(int)*MAXNA);
            t=0;for(int i=0;i<na;i++)t+=hc[i];pa_dcs+=(float)t/na;
        }
        pa_nodcs/=3;pa_dcs/=3;
        printf("%5d | %11.1f | %9.1f | %.2fx\\n",na,pa_nodcs,pa_dcs,pa_dcs/pa_nodcs);
    }
    return 0;
}
