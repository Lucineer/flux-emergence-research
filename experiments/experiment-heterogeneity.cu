...
__global__ void init_w(int seed,int mode){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;
    aseed[i]=seed+i*137;ax[i]=rn(&aseed[i])%SZ;ay[i]=rn(&aseed[i])%SZ;acol[i]=0;
    if(mode==0)aspeed[i]=1;       // uniform slow
    else if(mode==1)aspeed[i]=2;   // uniform medium
    else if(mode==2)aspeed[i]=i%4+1; // mixed 1-4
    else aspeed[i]=rn(&aseed[i])%4+1; // random 1-4
}
...
int main(){
    ...
    printf("=== Agent Heterogeneity x DCS ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    printf("Mode        | NoDCS | DCS | Lift\\n");
    printf("----------------------------------\\n");
    char*names[]={"Uniform-slow","Uniform-med","Mixed-1-4","Random-1-4"};
    for(int mode=0;mode<4;mode++){
        float pa_nodcs=0,pa_dcs=0;
        for(int trial=0;trial<3;trial++){
            int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<32,BLK>>>(42+trial,mode);init_f<<<fb,BLK>>>(999+trial);cudaDeviceSynchronize();
