...
__global__ void do_resp(int unused){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=FOOD)return;
    if(!falive[i]){ftimer[i]+=1.0f;if(ftimer[i]>respawn_rate){falive[i]=1;ftimer[i]=0;}}
}
...
int main(){
    ...
    printf("=== Food Respawn Rate x DCS ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    printf("Respawn | NoDCS | DCS | Lift\\n");
    printf("------------------------------\\n");
    float rates[]={10.0f,25.0f,50.0f,100.0f,200.0f,500.0f};
    for(int ri=0;ri<6;ri++){
        cudaMemcpyToSymbol(respawn_rate,&rates[ri],sizeof(float));
        ...
        pa_nodcs/=3;pa_dcs/=3;
        printf(" %5.0f | %5.0f | %3.0f | %.2fx\\n",rates[ri],pa_nodcs,pa_dcs,pa_dcs/pa_nodcs);
    }
    return 0;
}
