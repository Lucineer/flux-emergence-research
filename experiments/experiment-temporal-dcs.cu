...
__device__ unsigned long long window_col[1];
...
__global__ void ss(int step,int use_dcs){
    ...
    if(use_dcs&&dcs_v[0]){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            if(dd<=g2&&bf>=0&&falive[bf]){falive[bf]=0;acol[i]++;atomicAdd(window_col,(unsigned long long)1);dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
            return;
        }
    }
    if(bf>=0&&bd<=g2){if(falive[bf]){falive[bf]=0;acol[i]++;atomicAdd(window_col,(unsigned long long)1);dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}}
...
int main(){
    ...
    printf("=== Temporal DCS Value (per 500-step window) ===\\n");
    ...
    for(int mode=0;mode<2;mode++){
        ...
        printf("%s:",mode?"DCS":"NoDCS");
        for(int w=0;w<6;w++){
            unsigned long long zw=0;cudaMemcpyToSymbol(window_col,&zw,sizeof(unsigned long long));
            for(int s=w*500;s<(w+1)*500;s++){ss<<<32,BLK>>>(s,mode);do_resp<<<fb,BLK>>>(0);if(s%200==0)cudaDeviceSynchronize();}
            cudaDeviceSynchronize();
            unsigned long long wc;cudaMemcpyFromSymbol(&wc,window_col,sizeof(unsigned long long));
            printf(" %5llu",wc);
        }
        printf("\\n");
    }
