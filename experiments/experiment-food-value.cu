...
__global__ void init_f(int seed,int mode){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=FOOD)return;
    int s=seed+i*777;fx[i]=s%SZ;fy[i]=(s*31)%SZ;falive[i]=1;ftimer[i]=0;
    if(mode==0)fval[i]=1;        // uniform
    else if(mode==1)fval[i]=(rn(&s)%5)+1; // varied 1-5
    else fval[i]=(i%10==0)?5:1;  // rare jackpot 5x
}
...
__global__ void ss(int step,int use_dcs){
    ...
    if(use_dcs&&dcs_v[0]){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            if(dd<=g2&&bf>=0&&falive[bf]){falive[bf]=0;acol[i]+=fval[bf];dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
            return;
        }
    }
    if(bf>=0&&bd<=g2){
        if(falive[bf]){falive[bf]=0;acol[i]+=fval[bf];dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
    }else if(bf>=0){
...
int main(){
    ...
    printf("=== Food Value Variation x DCS ===\\n");
    printf("Value Mode | NoDCS | DCS | Lift\\n");
    printf("----------------------------------\\n");
    char*names[]={"Uniform(1)","Varied(1-5)","Jackpot(5x)"};
    for(int mode=0;mode<3;mode++){
        float pa_nodcs=0,pa_dcs=0;
        for(int trial=0;trial<3;trial++){
            int z[1];z[0]=0;cudaMemcpyToSymbol(dcs_v,z,sizeof(int));
            init_w<<<32,BLK>>>(42+trial);init_f<<<fb,BLK>>>(999+trial,mode);cudaDeviceSynchronize();
