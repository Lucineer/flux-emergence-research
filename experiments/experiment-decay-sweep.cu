...
// MEMORY WITH DECAY: Does forgetting fix the memory penalty?
// Modes: No memory, Memory (no decay), Decay-10%, Decay-25%, Decay-50%
// Hypothesis: Forgetting rate should match regeneration rate

__device__ int rt[R],ri[R][IT],re[R][EX],rg[R];
__device__ int ar[A],ah[A],ag[A],as[A],al[A],aseed[A];
__device__ int amode[A];
__device__ int pers_mem[A][R];
...
__global__ void step(){
    ...
    int exp_delta=0;
    if(rt[room]==0)exp_delta=-3;
    else if(rg[room]>20)exp_delta=2;
    else if(rg[room]<5)exp_delta=-1;
    
    // Update memory with decay
    int decay;
    switch(mode){
        case 0: decay=0;break; // no memory
        case 1: decay=0;break; // no decay
        case 2: decay=10;break; // 10% decay
        case 3: decay=25;break; // 25% decay
        case 4: decay=50;break; // 50% decay
    }
    
    if(mode>=1){
        pers_mem[i][room]=mx(-5,mn(10,pers_mem[i][room]+exp_delta));
        // Apply decay to ALL rooms
        if(decay>0){
            for(int r=0;r<R;r++){
                pers_mem[i][r]=pers_mem[i][r]*(100-decay)/100;
            }
        }
    }
...
int main(){
    printf("=== Memory Decay Sweep ===\n");
    printf("128 rooms, 256 agents (52 each), 500 steps, 64 trials\n\n");
    const char* nm[]={"No-Memory","Mem-No-Decay","Mem-10%%","Mem-25%%","Mem-50%%"};
    float tot[NA][3]={0};
    ...
    printf("\nvs No-Memory:\n");
    for(int m=1;m<NA;m++) printf("  %s: %+.1f%% score\n",nm[m],(tot[m][0]/tot[0][0]-1)*100);
    return 0;
}
