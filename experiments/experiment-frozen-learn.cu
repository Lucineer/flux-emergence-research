...
__global__ void init_a(int seed){
    ...
    // Identity init with small noise
...
__global__ void step(){
    ...
    for(int e=0;e<EX;e++){
        int nr=re[room][e];float nl[4];enc(nr,nl);
        float sc=0;
        switch(mode){
            case 0: // Hardcoded
                {int r=0;for(int j=0;j<IT;j++)if(ri[nr][j]>0)r++;sc=r*2.0f+rg[nr]*0.1f-rt[nr]*0.5f;}
                break;
            case 1: // Frozen identity: use features directly, no learning
                sc=nl[1]*2.0f+nl[2]*0.5f-nl[3]*3.0f;
                break;
            case 2: // Prediction-guided, frozen weights
                ...
                break;
            case 3: // Prediction-guided, SLOW learning (lr=0.001)
                for(int r=0;r<4;r++){pred[r]=0;for(int c=0;c<4;c++)pred[r]+=a_w[i][r*4+c]*a_lat[i][c];pred[r]=fminf(1.0f,fmaxf(-1.0f,pred[r]));}
                sc=pred[1]*3.0f+pred[2]*2.0f-pred[3]*5.0f;
                break;
            case 4: // Hybrid: use prediction if confident, else hardcoded
                for(int r=0;r<4;r++){pred[r]=0;for(int c=0;c<4;c++)pred[r]+=a_w[i][r*4+c]*a_lat[i][c];pred[r]=fminf(1.0f,fmaxf(-1.0f,pred[r]));}
                {float conf=0;for(int r=0;r<4;r++)conf+=pred[r]*pred[r];conf=sqrtf(conf/4.0f);
                if(conf>0.3f) sc=pred[1]*3.0f+pred[2]*2.0f-pred[3]*5.0f;
                else {int r=0;for(int j=0;j<IT;j++)if(ri[nr][j]>0)r++;sc=r*2.0f+rg[nr]*0.1f-rt[nr]*0.5f;}}
                break;
        }
        if(sc>bs){bs=sc;be=e;}
    }
    int nw=re[room][be];ar[i]=nw;
    // Learning only for modes 3-4
    if(mode==3||mode==4){
        float act[4];enc(nw,act);float lr=0.001f;
        for(int r=0;r<4;r++){float e=act[r]-pred[r];for(int c=0;c<4;c++)a_w[i][r*4+c]+=lr*e*a_lat[i][c];}
    }
    // Update latent for all JEPA modes
    if(mode>=1){float act[4];enc(nw,act);for(int r=0;r<4;r++)a_lat[i][r]=act[r];}
...
int main(){
    printf("=== Frozen vs Learned Weights + Confidence Hybrid ===\n");
    printf("256 agents (52 each), 128 rooms, 500 steps, 64 trials\n\n");
    const char* nm[]={"Hardcoded","Frozen-Features","Frozen-Pred","Slow-Learn(0.001)","Confidence-Hybrid"};
    float tot[NA][3]={0};
