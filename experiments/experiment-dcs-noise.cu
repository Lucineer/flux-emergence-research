...
__global__ void ss(int step,int noise_pct){
    // noise_pct: 0=no dcs, 1=perfect dcs, 2-50=noisy dcs (X% chance of random offset)
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;
    int g2=144;
    int bd=999999,bf=-1;
    for(int f=0;f<FOOD;f++){if(!falive[f])continue;int d=td(ax[i],ay[i],fx[f],fy[f]);if(d<bd){bd=d;bf=f;}}
    if(noise_pct>0&&dcs_v[0]){
        int px=dcs_x[0],py=dcs_y[0];
        // Add noise
        if(noise_pct>1){
            int n=noise_pct;
            int r1=rn(&aseed[i]);int r2=rn(&aseed[i]);
            if(r1%100<n){px=(px+r2%(2*n)-n+SZ)%SZ;py=(py+r2%(2*n)-n+SZ)%SZ;}
        }
        int dd=td(ax[i],ay[i],px,py);
        if(dd<g2*4&&bd>dd){
...
int main(){
    ...
    printf("=== DCS Noise Tolerance ===\\n");
    printf("4096 agents, 400 food, 256x256, grab=12\\n");
    printf("Noise%% | Collection/agent | vs NoDCS\\n");
    printf("---------------------------------------\\n");
    int noises[]={0,1,5,10,20,30,50};
    float base_pa=0;
    for(int ni=0;ni<7;ni++){
        ...
        for(int trial=0;trial<3;trial++){
            ...
            for(int s=0;s<STEPS;s++){ss<<<32,BLK>>>(s,noises[ni]);do_resp<<<fb,BLK>>>(0);if(s%500==0)cudaDeviceSynchronize();}
        ...
        pa/=3;
        if(ni==0)base_pa=pa;
        printf("  %3d  | %15.1f |",noises[ni],pa);
        if(ni==0)printf(" baseline\\n");else printf(" %.2fx\\n",pa/base_pa);
    }
    return 0;
}
