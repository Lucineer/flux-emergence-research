...
__global__ void ss(int step,int mode){
    // 0=none, 1=dcs, 2=stigmergy, 3=both, 4=stig+noise5, 5=stig+noise20, 6=stig+migrating
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;
    int g2=144;
    int bd=999999,bf=-1;
    ...
    if((mode==1||mode==3)&&dcs_v[0]){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            if(dx!=0||dy!=0){ax[i]=to(ax[i]+dx/2);ay[i]=to(ay[i]+dy/2);moved=1;}
            if(dd<=g2&&bf>=0&&falive[bf]){falive[bf]=0;acol[i]++;heat[to(fy[bf])*SZ+to(fx[bf])]+=10;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}
        }
    }
    
    if(!moved&&(mode==2||mode==3||mode==4||mode==5||mode==6)){
        int bx=0,by=0;
        for(int dx=-3;dx<=3;dx+=3){
            for(int dy=-3;dy<=3;dy+=3){
                int nx=to(ax[i]+dx),ny=to(ay[i]+dy);
                int h=heat[ny*SZ+nx];
                // Add noise for modes 4,5
                if(mode==4)h+=(rn(&aseed[i])%20)-10;
                if(mode==5)h+=(rn(&aseed[i])%80)-40;
                bx+=h*dx;by+=h*dy;
            }
        }
    ...
    if(!moved){
        if(bf>=0&&bd<=g2){
            if(falive[bf]){falive[bf]=0;acol[i]++;heat[to(fy[bf])*SZ+to(fx[bf])]+=10;if(mode==1||mode==3){dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}}
        }else if(bf>=0){
...
__global__ void migrate_f(int step){
    // Mode 6: slowly move food
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=FOOD||!falive[i])return;
    fx[i]=to(fx[i]+((step+i)%3)-1);
    fy[i]=to(fy[i]+((step+i*3)%3)-1);
}
int main(){
    int fb=(FOOD+BLK-1)/BLK;int hb=(SZ*SZ+BLK-1)/BLK;
    printf("=== Stigmergy Robustness Test ===\\n");
    printf("4096 agents, 400 food, grab=12\\n");
    printf("Mode           | Collection | vs None\\n");
    printf("--------------------------------------\\n");
    char*names[]={"None","DCS","Stigmergy","DCS+Stig","Stig+Noise5","Stig+Noise20","Stig+Migrate"};
    float pa[7];
    for(int mode=0;mode<7;mode++){
        ...
        for(int trial=0;trial<3;trial++){
            ...
            for(int s=0;s<STEPS;s++){
                if(mode==6)migrate_f<<<fb,BLK>>>(s);
                ss<<<32,BLK>>>(s,mode);
                do_resp<<<fb,BLK>>>(0);
                if(s%10==0)decay_heat<<<hb,BLK>>>();
    ...
    for(int m=0;m<7;m++){
        if(m==0)printf("  %-14s | %10.1f | baseline\\n",names[m],pa[m]);
        else printf("  %-14s | %10.1f | %.2fx\\n",names[m],pa[m],pa[m]/pa[0]);
    }
    return 0;
}
