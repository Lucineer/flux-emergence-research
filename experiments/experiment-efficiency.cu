...
#define BLK 128
#define STEPS 3000
__device__ int ax[NA],ay[NA],acol[NA],aseed[NA],asteps[NA];
...
__global__ void init_w(int seed){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=NA)return;aseed[i]=seed+i*137;ax[i]=rn(&aseed[i])%SZ;ay[i]=rn(&aseed[i])%SZ;acol[i]=0;asteps[i]=0;}
...
__global__ void ss(int step,int use_dcs){
    ...
    if(use_dcs&&dcs_v[0]){
        ...
        if(dd<g2*4&&bd>dd){
            ...
            if(dx!=0||dy!=0){ax[i]=to(ax[i]+dx/2);ay[i]=to(ay[i]+dy/2);asteps[i]++;}
    ...
    if(bf>=0&&bd<=g2){if(falive[bf]){falive[bf]=0;acol[i]++;dcs_x[0]=fx[bf];dcs_y[0]=fy[bf];dcs_v[0]=1;}}
        ...
        if(dx<-SZ/2)dx+=SZ;if(dx>SZ/2)dx-=SZ;if(dy<-SZ/2)dy+=SZ;if(dy>SZ/2)dy-=SZ;
        if(dx!=0||dy!=0){ax[i]=to(ax[i]+dx);ay[i]=to(ay[i]+dy);asteps[i]++;}
    }
}
int main(){
    int fb=(FOOD+BLK-1)/BLK;
    printf("=== Agent Efficiency Analysis ===\\n");
    ...
    for(int mode=0;mode<2;mode++){
        ...
        int hc[NA],hs[NA];cudaMemcpyFromSymbol(hc,acol,sizeof(int)*NA);cudaMemcpyFromSymbol(hs,asteps,sizeof(int)*NA);
        long tcol=0,tsteps=0;
        float min_eff=9999,max_eff=0;
        for(int i=0;i<NA;i++){tcol+=hc[i];tsteps+=hs[i];
            float eff=(float)hc[i]/(hs[i]+1);
            if(eff<min_eff)min_eff=eff;if(eff>max_eff)max_eff=eff;
        }
        printf("%s:\\n",mode?"DCS":"NoDCS");
        printf("  Total: %ld collections, %ld steps moved\\n",tcol,tsteps);
        printf("  Avg efficiency: %.4f coll/step\\n",(float)tcol/tsteps);
        printf("  Per-agent efficiency range: %.4f to %.4f\\n",min_eff,max_eff);
        printf("  Efficiency ratio (max/min): %.1fx\\n\\n",max_eff/min_eff);
    }
    return 0;
}
