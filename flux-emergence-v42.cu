/* flux-emergence-v42.cu — Combined v2 (Coop + Cluster + Scarcity)
   Novel: stack v40 (cooperative carrying) + v25 (clustered spawn) + v8 (scarcity baseline).
   Prediction: coop adds to cluster advantage because clustered agents can more easily
   reach the 2-agent threshold for heavy resources. */
...
__global__ void init_r_coop(Resource*r,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].collected=0;r[i].heavy=(i<(int)(n*0.3f))?1:0;r[i].value=(r[i].heavy)?1.5f:(.5f+lcgf(&s)*.5f);}
__global__ void init_r_norm(Resource*r,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].value=.5f+lcgf(&s)*.5f;r[i].collected=0;r[i].heavy=0;}

__global__ void count_near(Resource*r,Agent*a,int nr,int na,int*nc){
    int j=blockIdx.x*blockDim.x+threadIdx.x;if(j>=nr||r[j].collected)return;
    int cnt=0;for(int i=0;i<na;i++){float dx=a[i].x-r[j].x,dy=a[i].y-r[j].y;if(sqrtf(dx*dx+dy*dy)<.03f)cnt++;}nc[j]=cnt;
}

__global__ void tick_s(Agent*a,Resource*r,int*nc,int na,int nr,int t,int pt,int coop,int cluster){
    ...
    if(br>=0&&bd<grab){
        int can_collect=1;
        if(coop&&r[br].heavy&&nc[br]<2)can_collect=0;
        if(can_collect){r[br].collected=1;ag->res_held++;
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v42 — Coop + Cluster + Scarcity (Combined v2)\n");
    printf("  A: coop+cluster  B: coop only  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    Agent*da,*ha,*db,*hb,*dc,*hc;Resource*dr;int*nc;
    cudaMalloc(&nc,NR*sizeof(int));
    ...
    float f_combo=0,f_coop=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        init_a_cluster<<<blk,256>>>(da,NA);init_r_coop<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){count_near<<<rblk,256>>>(dr,da,NR,NA,nc);tick_s<<<blk,256>>>(da,dr,nc,NA,NR,t,250,1,1);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_combo+=f;
        init_a_uniform<<<blk,256>>>(db,NA);init_r_coop<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){count_near<<<rblk,256>>>(dr,db,NR,NA,nc);tick_s<<<blk,256>>>(db,dr,nc,NA,NR,t,250,1,0);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_coop+=f;
        init_c<<<blk,256>>>(dc,NA);init_r_coop<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        ...
        printf("Exp %d: combo=%.1f coop=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_combo/=5;f_coop/=5;f_ctrl/=5;
    float synergy=(f_coop>.01f)?f_combo/f_coop:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Combo:  %.1f (%.2fx ctrl)\n",f_combo,(f_ctrl>.01f)?f_combo/f_ctrl:1);
    printf("  Coop:   %.1f (%.2fx ctrl)\n",f_coop,(f_ctrl>.01f)?f_coop/f_ctrl:1);
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Synergy: %.2fx (%+.0f%%)\n",synergy,(synergy-1)*100);
    printf("  Verdict: %s\n",synergy>1.1?"SYNERGY":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
