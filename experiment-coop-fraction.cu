/* experiment-coop-fraction.cu — What fraction of heavy resources is optimal?
   v40 used 30% heavy (3x value, need 2 agents). Sweep 0%, 10%, 20%, 30%, 50%, 70%.
   Prediction: too few = no coop benefit, too many = bottleneck (can't reach threshold). */
...
__global__ void tick_s(Agent*a,Resource*r,int*nc,int na,int nr,int t,int pt){
    ...
    if(br>=0&&bd<grab){
        if(r[br].heavy&&nc[br]<2){/* skip heavy, can't collect alone */}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  Experiment: Optimal Cooperative Fraction Sweep\n");
    printf("  What %% of resources should be heavy (team)?\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    // Get control baseline (no heavy resources)
    float f_ctrl=0;
    for(int e=0;e<5;e++){init_c<<<blk,256>>>(dc,NA);init_r<<<rblk,256>>>(dr,NR,0.0f);cudaDeviceSynchronize();for(int t=0;t<MAXT;t++){tick_c<<<blk,256>>>(dc,dr,NA,NR,t,250);cudaDeviceSynchronize();}cudaMemcpy(hc,dc,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=hc[i].fitness;f_ctrl+=f;}
    f_ctrl/=5;
    printf("Control (no heavy): %.1f\n\n",f_ctrl);
    
    float fracs[]={0.0f,0.1f,0.2f,0.3f,0.5f,0.7f};
    for(int fi=0;fi<6;fi++){
        float f=0;
        for(int e=0;e<5;e++){
            init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR,fracs[fi]);cudaDeviceSynchronize();
            for(int t=0;t<MAXT;t++){count_near<<<rblk,256>>>(dr,da,NR,NA,nc);tick_s<<<blk,256>>>(da,dr,nc,NA,NR,t,250);cudaDeviceSynchronize();}
            cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float ff=0;for(int i=0;i<NA;i++)ff+=ha[i].fitness;f+=ff;
        }
        f/=5;float ratio=(f_ctrl>.01f)?f/f_ctrl:1;
        printf("  Heavy %3.0f%%: %.1f (%.2fx ctrl)\n",fracs[fi]*100,f,ratio);
    }
    printf("\n═══════════════════════════════════════════════════════\n");
