...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    float speed_mult, float init_energy, int steps, int n, int food_count, int w, unsigned int seed) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    unsigned int rng = seed + tid * 997;
    float x = cr(&rng)*w, y = cr(&rng)*w, energy = init_energy, score = 0.0f;
    float base_angle = tid * 2.39996f;
    float dir[8];for(int i=0;i<8;i++)dir[i]=base_angle+i*0.785f;
    for(int t=0;t<steps&&energy>0;t++){
        int p=t%8;float dx=cosf(dir[p])*2.0f*speed_mult,dy=sinf(dir[p])*2.0f*speed_mult;
        ...
        for(int i=0;i<food_count;i++){
            ...
            if(fdx*fdx+fdy*fdy<16.0f){int old=atomicExch(&falive[i],0);if(old){energy=fminf(energy+10.0f,init_energy*1.5f);score+=1.0f;}}
        }
    }
...
int main(){
    float energies[NENERGY]={20.0f,30.0f,40.0f,60.0f,80.0f,100.0f,150.0f};
    float speeds[NSPEEDS]={1.0f,16.0f};
    printf("=== ENERGY CLIFF vs SPEED: Law 226 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int sp=0;sp<NSPEEDS;sp++){
        printf("--- Speed %dx ---\n",(int)speeds[sp]);
        printf("Energy | Score  | Survival\n");
        printf("-------|--------|----------\n");
        for(int e=0;e<NENERGY;e++){
            float ts=0,ta=0;
            for(int tr=0;tr<TRIALS;tr++){
                cudaMemset(d_fa,1,FOOD*sizeof(int));
                simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,speeds[sp],energies[e],STEPS,N,FOOD,W,(unsigned int)(42+tr*1111+sp*111+e*11));
            ...
            printf("%-6.0f | %5.3f  | %5.1f%%\n",energies[e],ts,surv);
        }
        printf("\n");
    }
    printf(">> Law 226: Does energy cliff shift with speed?\n");
