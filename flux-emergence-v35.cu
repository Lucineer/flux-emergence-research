/* flux-emergence-v35.cu — Age-Based Expertise (Accumulation Bonus)
   Novel: agents gain permanent grab-range multiplier from collections.
   Tests if experience compounds (law #2: accumulation > adaptation).
   Prediction: confirms accumulation, +10-20% from expertise curve. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,group;float tip_x,tip_y,tip_val;unsigned int rng;int age;float expertise;}Agent;
typedef struct{float x,y,value;int collected;}Resource;

__global__ void init_a(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].age=0;a[i].expertise=1.0f;for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}

__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].age=0;a[i].expertise=1.0f;for(int r=0;r<4;r++)a[i].role[r]=.25f;}
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int expertise_on){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];ag->age++;
    float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float grab=(.02f+cp*.02f)*(expertise_on?ag->expertise:1.0f);
    float det=.03f+ep*.04f;float bd=det;int br=-1;
    ...
    if(br>=0&&bd<grab){
        ...
        if(expertise_on)ag->expertise=fminf(2.0f,ag->expertise+.01f);
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v35 — Age-Based Expertise (Accumulation)\n");
    printf("  Grab range grows with collections, cap 2.0x\n");
    printf("  A: expertise  B: no expertise  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_exp+=f;
        ...
        float maxexp=0;for(int i=0;i<NA;i++)if(ha[i].expertise>maxexp)maxexp=ha[i].expertise;
        printf("Exp %d: expert=%.1f no=%.1f ctrl=%.1f max_exp=%.2f\n",e+1,f/NA,f/NA,f/NA,maxexp);
    }
    f_exp/=5;f_no/=5;f_ctrl/=5;
    float boost=(f_no>.01f)?f_exp/f_no:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Expertise: %.1f (%.2fx ctrl)\n",f_exp,(f_ctrl>.01f)?f_exp/f_ctrl:1);
    printf("  No expert: %.1f (%.2fx ctrl)\n",f_no,(f_ctrl>.01f)?f_no/f_ctrl:1);
    printf("  Control:   %.1f\n",f_ctrl);
    printf("  Boost:     %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"ACCUMULATION WINS":boost<0.95?"HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
