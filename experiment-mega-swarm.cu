// experiment-mega-swarm.cu — Maximum swarm: 128→16384 agents
// With timing to benchmark GPU performance
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void run(int n_agents, int seed) {
    ...
    float msteps = (float)n_agents * STEPS / 1e6;
    printf("  agents=%5d  total=%10.0f  per=%8.1f  time=%6.1fms  %.1f Msteps/s\n",
           n_agents, total, total/n_agents, ms, msteps/(ms/1000.0f));
}

int main() {
    printf("=== Mega Swarm — Timed GPU Benchmark ===\n");
    printf("400 food, 3000 steps, 1024x1024 world, Jetson Orin Nano\n\n");
