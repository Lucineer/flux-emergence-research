/* flux-emergence-v3.cu — Iteration 3: anti-convergence + verified grid.
   v2 finding: 10x coupling caused HOMOGENIZATION not specialization (0.002 CV).
   v3: moderate coupling + anti-convergence drift + verified interaction counts. */
...
__global__ void tick(Agent *a, Resource *r, int na, int nr, int t, int pt) {
    ...
    if (i >= na) return;
    Agent *ag = &a[i];
    float cw = ag->role[1], ew = ag->role[0];
    ...
    /* Neighbor interaction: sample random subset instead of grid */
    int ints = 0;
    for (int k = 0; k < 32; k++) {
        int j = lcg(&ag->rng) % na;
        if (j == i) continue;
        ...
        /* v3: MODERATE coupling (0.02 same, 0.002 cross) */
        float infl = (a[j].arch == ag->arch) ? 0.02f : 0.002f;
        for (int r = 0; r < 4; r++)
            ag->role[r] += (a[j].role[r] - ag->role[r]) * infl;

        /* v3: ANTI-CONVERGENCE DRIFT
           If roles are very similar to neighbor, add small random perturbation.
           This prevents clone formation while allowing alignment. */
        float sim = 0.0f;
        for (int r = 0; r < 4; r++) {
            float diff = ag->role[r] - a[j].role[r];
            sim += 1.0f - fminf(1.0f, fabsf(diff));
        }
        sim /= 4.0f;
        if (sim > 0.9f) {
            /* Too similar — drift the non-archetype roles randomly */
            int drift_role = (ag->arch + 1 + lcg(&ag->rng) % 3) % 4;
            ag->role[drift_role] += (lcgf(&ag->rng) - 0.5f) * 0.01f;
        }
    ...
    /* v3: Role energy bonus — archetype role earns energy */
...
int main() {
    printf("═══════════════════════════════════════════════════\n");
    printf("  FLUX EMERGENCE v3 — Anti-Convergence\n");
    printf("  v1: spec=0.019, v2: spec=0.002 (homogenized)\n");
    printf("  v3: moderate coupling + drift to prevent clones\n");
    printf("═══════════════════════════════════════════════════\n\n");
    ...
    printf("\n═══════════════════════════════════════════════════\n");
    printf("  COMPARISON: v1=baseline v2=strong v3=anti-conv\n");
    printf("  Clustering:     %.3f %s  (v1:0.397 v2:0.413)\n",as, as>0.3?"OK":"FAIL");
    printf("  Specialization: %.3f %s  (v1:0.019 v2:0.002)\n",asp, asp>0.2?"YES":asp>0.1?"~":"NO");
    printf("  Efficiency:     %.2fx %s  (v1:1.30x v2:1.30x)\n",ae, ae>1.5?"YES":"~");
