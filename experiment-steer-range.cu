...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float mark_range, unsigned int seed) {
    ...
    float rr=mark_range*mark_range;
    for(int t=0;t<steps&&energy>0;t++){
        ...
        for(int m=0;m<nmarks;m++){
            ...
            float d=mx*mx+my*my;
            if(d<rr&&d>0.01f){sx-=mx/sqrtf(d);sy-=my/sqrtf(d);}
        }
...
int main(){
    float ranges[NRANGES]={5.0f,10.0f,20.0f,30.0f,50.0f,100.0f};
    printf("=== MARK INFLUENCE RANGE: Law 235 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int r=0;r<NRANGES;r++){
        ...
        ts/=TRIALS;tt/=TRIALS;
        if(ts>best){best=ts;bi=r;}
        printf("Range %-4.0f: score=%.3f fleet=%.1f\n",ranges[r],ts,tt);
    }
    printf("\nBest: range %.0f (score=%.3f)\n",ranges[bi],best);
