// Experiment: Territory Stigmergy vs Food Stigmergy
// Law 192: Does "depleted territory" marking beat "food location" marking?
// Hypothesis: Negative marks (visited=depleted) beat positive marks (food=here)
// Key: Agents AVOID marked territory, forcing exploration of NEW areas
...
#define STEPS 2000
#define BLK 128
#define NGRID 32  // 32x32 territory grid (8x8 cells each)

typedef struct { float x,y; int alive; float energy; } Agent;
typedef struct { float x,y; int active; } Food;

// Territory grid: how many times each cell has been visited
__device__ int territory[NGRID*NGRID];
...
__global__ void init(Agent *ag, Food *fd, int seed) {
    ...
    if(i<NGRID*NGRID) territory[i]=0;
}
...
__global__ void reset_territory() {
    int i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i<NGRID*NGRID) territory[i]=0;
}

// mode: 0=none, 1=food-mark(positive), 2=territory-avoid(negative), 3=territory-repulsion
__global__ void step(Agent *ag, Food *fd, int mode, float grab, float perceive, int seed) {
    int i = blockIdx.x*blockDim.x+threadIdx.x;
    ...
    // Get current territory cell
    int gc = (int)(ag[i].x / (float)N * NGRID) % NGRID;
    int gr = (int)(ag[i].y / (float)N * NGRID) % NGRID;
    atomicAdd(&territory[gr*NGRID+gc], 1);
    
    // Base target: random walk
    float tx = ag[i].x + (r1-0.5f)*12.0f;
    float ty = ag[i].y + (r2-0.5f)*12.0f;
    
    // Mode 2/3: territory avoidance — prefer LESS visited cells
    if(mode >= 2) {
        float best_score = FLT_MAX;
        for(int dr=-2;dr<=2;dr++) {
            for(int dc=-2;dc<=2;dc++) {
                int nr = ((gr+dr)%NGRID+NGRID)%NGRID;
                int nc = ((gc+dc)%NGRID+NGRID)%NGRID;
                int visits = territory[nr*NGRID+nc];
                // Score: lower visits = better target
                float score = (float)visits;
                if(mode==3) score += (abs(dr)+abs(dc))*2.0f; // prefer nearby cells
                if(score < best_score) {
                    best_score = score;
                    tx = ((float)nc + 0.5f) / (float)NGRID * (float)N;
                    ty = ((float)nr + 0.5f) / (float)NGRID * (float)N;
                }
            }
        }
    }
...
void run(const char *label, int mode, int seed) {
    ...
    int nbg=(NGRID*NGRID+BLK-1)/BLK;
    
    init<<<((1024)+BLK-1)/BLK,BLK>>>(d_ag,d_fd,seed);
    init_patches<<<nbp,BLK>>>(d_fd,seed+999);
    reset_territory<<<nbg,BLK>>>();
    
    for(int t=0;t<STEPS;t++) {
        step<<<nb,BLK>>>(d_ag,d_fd,mode,2.0f,4.0f,seed+t);
    }
    ...
    printf("  %-20s: score=%.0f alive=%d food_left=%d\n", label, score, alive, fl);
    
    cudaFree(d_ag); cudaFree(d_fd);
    cudaFree(d_s); cudaFree(d_a); cudaFree(d_fl);
}

int main() {
    printf("=== Territory Stigmergy vs Food Stigmergy ===\n");
    printf("64 agents, 400 food (20 patches), perceive=4.0, grab=2.0\n");
    printf("32x32 territory grid, 2000 steps\n\n");
    
    for(int t=0;t<5;t++) {
        int seed=42+t*1000;
        printf("Trial %d:\n",t+1);
        run("no-stigmergy", 0, seed);
        run("territory-avoid", 2, seed);
        run("territory-nearby", 3, seed);
        printf("\n");
    }
    return 0;
}
