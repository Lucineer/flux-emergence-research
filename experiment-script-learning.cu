...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int use_bias, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        int p=t%8;
        float dx=cosf(dir[p]+bias_angle)*2.0f, dy=sinf(dir[p]+bias_angle)*2.0f;
        ...
        for(int i=0;i<food_count;i++){
            ...
            if(fdx*fdx+fdy*fdy<16.0f){int old=atomicExch(&falive[i],0);if(old){
                energy=fminf(energy+10.0f,200.0f);score+=1.0f;
                if(use_bias){
                    float fa=atan2f(fy[i]-oq,fx[i]-ox);
                    float diff=fa-bias_angle;
                    while(diff>3.14159f)diff-=6.2832f;
                    while(diff<-3.14159f)diff+=6.2832f;
                    bias_angle+=diff*0.2f; // 20% bias toward food
                }
            }}
        }
        if(use_bias) bias_angle*=0.995f; // slow decay
    }
    scores[tid]=score;alive[tid]=(energy>0)?1:0;
}

int main(){
    const char* nm[]={"PureScript","Script+Bias"};
    printf("=== SCRIPT LEARNING BIAS: Law 232 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    float r0=0,r1=0; // recalculate for comparison
    printf("\n>> Law 232: Does directional bias (learning) help scripted agents?\n");
