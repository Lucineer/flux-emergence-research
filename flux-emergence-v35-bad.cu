/* flux-emergence-v35.cu — Age-Based Expertise (Accumulation Bonus)
   Novel: agents gain a permanent "expertise" multiplier based on how long they've
   been alive and collecting. Older agents with more collections get a grab range bonus.
   Tests if experience/accumulation compounds over time (matches law #2: accumulation > adaptation).
   Prediction: confirms accumulation advantage, +10-20% from expertise curve. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int expertise_on){
    ...
    Agent*ag=&a[i];
    ag->age++;
    /* v35: expertise grows with collections, caps at 2.0x */
    float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float grab_base=.02f+cp*.02f;
    float grab=grab_base*(expertise_on?ag->expertise:1.0f);
    ...
    if(br>=0&&bd<grab){
        ...
        /* Expertise: each collection increases by 0.01, log curve, cap at 2.0 */
    ...
    if(t==pt){ag->energy*=(1-.5f*(1-df*.5f));ag->x=lcgf(&ag[i].rng);ag->y=lcgf(&ag->rng);ag->vx=ag->vy=0;ag->tip_val=0;}
}
