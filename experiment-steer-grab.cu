...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int use_steer, float grab_r, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        for(int i=0;i<food_count;i++){
            ...
            if(fdx*fdx+fdy*fdy<rr){int old=atomicExch(&falive[i],0);if(old){energy=fminf(energy+10.0f,200.0f);score+=1.0f;}}
        }
    }
...
int main(){
    printf("=== STEER + GRAB COMBINATION: Law 225 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    int blk=(N+BLK-1)/BLK;
    int modes_steer[]={0,1,0,1};
    float modes_grab[]={4.0f,4.0f,12.0f,12.0f};
    const char* nm[]={"PureScript","Steer","BigGrab","Steer+BigGrab"};
    float results[NMODES];
    for(int m=0;m<NMODES;m++){
        float ts=0,ta=0,tt=0;
        for(int tr=0;tr<TRIALS;tr++){
            cudaMemset(d_fa,1,FOOD*sizeof(int));
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,modes_steer[m],modes_grab[m],(unsigned int)(42+tr*1111+m*111));
        ...
        results[m]=ts/TRIALS;float surv=ta/TRIALS/N*100;tt/=TRIALS;
        printf("%-16s: score=%.3f surv=%.1f%% fleet=%.1f\n",nm[m],results[m],surv,tt);
    }
    float base=results[0];
    float steer_only=results[1],grab_only=results[2],combo=results[3];
    float steer_lift=(steer_only-base)/base*100;
    float grab_lift=(grab_only-base)/base*100;
    float combo_lift=(combo-base)/base*100;
    float expected_additive=base*(1+steer_lift/100)*(1+grab_lift/100)-base;
    printf("\nLifts from baseline:\n");
    printf("  Steer only:     %+.1f%%\n",steer_lift);
    printf("  BigGrab only:   %+.1f%%\n",grab_lift);
    printf("  Combo:          %+.1f%%\n",combo_lift);
    printf("  Expected add:   %+.1f%%\n",expected_additive/base*100);
    if(combo_lift < expected_additive/base*100*0.9)
        printf(">> Law 225: DIMINISHING — mechanisms overlap (steer spreads, grab collects)\n");
    else
        printf(">> Law 225: ADDITIVE — mechanisms are independent\n");
