/* experiment-grab-x-coop.cu — Does grab range amplify cooperative carrying?
   v38 showed grab range is master lever. v40 showed coop helps.
   Question: at high grab range, does coop still help? Or does grab range make coop irrelevant?
   2x2 matrix: grab(1x vs 2x) × coop(none vs 30% heavy) */
...
typedef struct{float x,y,value;int collected,heavy;}Resource;
...
__global__ void init_r(Resource*r,int n,int heavy){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].collected=0;r[i].heavy=(heavy&&i<(int)(n*0.3f))?1:0;r[i].value=r[i].heavy?1.5f:(.5f+lcgf(&s)*.5f);}
__global__ void count_near(Resource*r,Agent*a,int nr,int na,int*nc){int j=blockIdx.x*blockDim.x+threadIdx.x;if(j>=nr||r[j].collected)return;int cnt=0;for(int i=0;i<na;i++){float dx=a[i].x-r[j].x,dy=a[i].y-r[j].y;if(sqrtf(dx*dx+dy*dy)<.03f)cnt++;}nc[j]=cnt;}
__global__ void tick_s(Agent*a,Resource*r,int*nc,int na,int nr,int t,int pt,float grab_mult){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float det=.03f+ep*.04f,grab=(.02f+cp*.02f)*grab_mult;float bd=det;int br=-1;
    ...
    if(br>=0&&bd<grab){
        if(r[br].heavy&&nc[br]<2){/* can't collect heavy alone */}
        else{r[br].collected=1;ag->res_held++;float tb=1;for(int k=0;k<16;k++){int j=lcg(&ag->rng)%na;if(j==i||a[j].arch!=ag->arch)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y;if(sqrtf(dx*dx+dy*dy)<.05f)tb+=a[j].role[3]*.2f;}float bn=(1+cp*.5f)*tb;ag->energy=fminf(1,ag->energy+r[br].value*.1f*bn);ag->fitness+=r[br].value*bn;}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  Experiment: Grab Range × Cooperation Interaction\n");
    printf("  2×2 matrix: grab(1x,2x) × coop(none,30%% heavy)\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    Agent*da,*ha,*dc,*hc;Resource*dr;int*nc;
    cudaMalloc(&nc,NR*sizeof(int));cudaMalloc(&da,NA*sizeof(Agent));cudaMalloc(&dc,NA*sizeof(Agent));cudaMalloc(&dr,NR*sizeof(Resource));
    ha=(Agent*)malloc(NA*sizeof(Agent));hc=(Agent*)malloc(NA*sizeof(Agent));
    int blk=(NA+255)/256,rblk=(NR+255)/256;
    float f_ctrl=0;
    for(int e=0;e<5;e++){init_c<<<blk,256>>>(dc,NA);init_r<<<rblk,256>>>(dr,NR,0);cudaDeviceSynchronize();for(int t=0;t<MAXT;t++){tick_c<<<blk,256>>>(dc,dr,NA,NR,t,250);cudaDeviceSynchronize();}cudaMemcpy(hc,dc,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=hc[i].fitness;f_ctrl+=f;}
    f_ctrl/=5;
    printf("Control: %.1f\n\n",f_ctrl);
    
    // 4 conditions: grab_mult × heavy
    struct Cond{float grab;int heavy;const char*name;} conds[]={
        {1.0f,0,"1x grab, no coop"},
        {1.0f,1,"1x grab, 30%% heavy"},
        {2.0f,0,"2x grab, no coop"},
        {2.0f,1,"2x grab, 30%% heavy"},
    };
    float results[4]={0};
    for(int ci=0;ci<4;ci++){
        for(int e=0;e<5;e++){
            init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR,conds[ci].heavy);cudaDeviceSynchronize();
            for(int t=0;t<MAXT;t++){count_near<<<rblk,256>>>(dr,da,NR,NA,nc);tick_s<<<blk,256>>>(da,dr,nc,NA,NR,t,250,conds[ci].grab);cudaDeviceSynchronize();}
            cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;results[ci]+=f;
        }
        results[ci]/=5;
        printf("  %-22s: %.1f (%.2fx ctrl)\n",conds[ci].name,results[ci],(f_ctrl>.01f)?results[ci]/f_ctrl:1);
    }
    printf("\n─── Interaction Analysis ───\n");
    float grab_boost=results[2]/results[0];
    float coop_boost_1x=results[1]/results[0];
    float coop_boost_2x=results[3]/results[2];
    printf("  Grab boost (no coop):   %.2fx\n",grab_boost);
    printf("  Coop boost at 1x grab:  %.2fx\n",coop_boost_1x);
    printf("  Coop boost at 2x grab:  %.2fx\n",coop_boost_2x);
    if(coop_boost_2x>coop_boost_1x)printf("  → Coop AMPLIFIED by larger grab range (synergy)\n");
    else if(coop_boost_2x<coop_boost_1x)printf("  → Coop DIMINISHED by larger grab range (redundant)\n");
    else printf("  → Coop effect INDEPENDENT of grab range\n");
    printf("═══════════════════════════════════════════════════════\n");
    cudaFree(da);cudaFree(dc);cudaFree(dr);cudaFree(nc);free(ha);free(hc);return 0;
}
