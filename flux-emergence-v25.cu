/* flux-emergence-v25.cu — Variable Agent Density (Clustered Spawn)
   Novel: agents spawn in 4 tight clusters (one per archetype), not uniformly.
   Tests if initial spatial clustering creates stronger territorial effects.
   Prediction: clustered spawn amplifies specialist advantage by 15%+. */
...
/* Cluster centers: 4 corners of unit square */
...
__global__ void init_a_uniform(Agent*a,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;
    a[i].rng=(unsigned int)(i*2654435761u+17);
    a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);
    a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;
    a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;
    a[i].tip_x=a[i].tip_y=a[i].tip_val=0;
    for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v25 — Variable Density (Clustered Spawn)\n");
    printf("  Agents spawn in 4 archetype clusters vs uniform\n");
    printf("  A: clustered  B: uniform  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_cl=0,f_un=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        init_a_cluster<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_cl+=f;
        init_a_uniform<<<blk,256>>>(db,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        ...
        printf("Exp %d: cluster=%.1f uniform=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_cl/=5;f_un/=5;f_ctrl/=5;
    float r_cl=(f_ctrl>.01f)?f_cl/f_ctrl:1,r_un=(f_ctrl>.01f)?f_un/f_ctrl:1;
    float boost=(f_un>.01f)?f_cl/f_un:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Clustered: %.1f (%.2fx ctrl)\n",f_cl,r_cl);
    printf("  Uniform:   %.1f (%.2fx ctrl)\n",f_un,r_un);
    printf("  Control:   %.1f\n",f_ctrl);
    printf("  Cluster boost: %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"CLUSTERING WINS":boost<0.9?"CLUSTERING HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
