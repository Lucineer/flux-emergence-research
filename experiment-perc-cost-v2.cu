...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    float speed_mult, float perc_cost, int steps, int n, int food_count, int w, unsigned int seed) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    unsigned int rng = seed + tid * 997;
    float x = cr(&rng)*w, y = cr(&rng)*w, energy = 150.0f, score = 0.0f;
    for(int t=0;t<steps&&energy>0;t++){
        energy -= perc_cost; // ABSOLUTE cost, not scaled
        float dx,dy,best_d=1e10f,bx=x,by=y;
        ...
        } else {dx=(cr(&rng)-0.5f)*4.0f;dy=(cr(&rng)-0.5f)*4.0f;}
...
int main(){
    float costs[NCOSTS]={0.0f,0.005f,0.01f,0.02f,0.03f,0.04f,0.05f,0.1f};
    float speeds[NSPEEDS]={1.0f,8.0f,32.0f};
    printf("=== PERCEPTION COST (ABSOLUTE): Law 216 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
