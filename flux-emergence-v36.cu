/* flux-emergence-v36.cu — Resource Memory Map (Cognitive Map)
   Novel: agents remember WHERE they found resources and boost detection range
   in those areas. Each agent tracks last 4 find locations; detection radius
   expands 2x within 0.05 of a remembered spot.
   Prediction: cognitive maps let agents re-exploit known productive areas. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,group;float tip_x,tip_y,tip_val;unsigned int rng;float mem_x[N_MEM],mem_y[N_MEM];int n_mem;}Agent;
typedef struct{float x,y,value;int collected;}Resource;

__device__ void add_mem(Agent*ag,float mx,float my){
    if(ag->n_mem<N_MEM){ag->mem_x[ag->n_mem]=mx;ag->mem_y[ag->n_mem]=my;ag->n_mem++;}
    else{int oldest=ag->n_mem%N_MEM;ag->mem_x[oldest]=mx;ag->mem_y[oldest]=my;ag->n_mem++;}
}

__device__ float mem_boost(Agent*ag){
    float mx=0;
    for(int m=0;m<N_MEM&&m<ag->n_mem;m++){
        float dx=ag->mem_x[m]-ag->x,dy=ag->mem_y[m]-ag->y;
        float d=sqrtf(dx*dx+dy*dy);if(d<.05f)mx=2.0f;
    }
    return mx;
}

__global__ void init_a(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].n_mem=0;for(int m=0;m<N_MEM;m++){a[i].mem_x[m]=0;a[i].mem_y[m]=0;}for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}

__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].n_mem=0;for(int m=0;m<N_MEM;m++){a[i].mem_x[m]=0;a[i].mem_y[m]=0;}for(int r=0;r<4;r++)a[i].role[r]=.25f;}
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int cmap){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float det_base=.03f+ep*.04f;
    float det=(cmap?det_base*mem_boost(ag):det_base);
    float grab=.02f+cp*.02f;float bd=det;int br=-1;
    ...
    if(br>=0&&bd<grab){
        ...
        if(cmap)add_mem(ag,r[br].x,r[br].y);
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v36 — Resource Memory Map (Cognitive Map)\n");
    printf("  Detection 2x in areas where agent previously found resources\n");
    printf("  A: cognitive map  B: no map  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_map+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_no+=f;
        ...
        printf("Exp %d: cmap=%.1f no=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_map/=5;f_no/=5;f_ctrl/=5;
    float boost=(f_no>.01f)?f_map/f_no:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Cognitive: %.1f (%.2fx ctrl)\n",f_map,(f_ctrl>.01f)?f_map/f_ctrl:1);
    printf("  No map:    %.1f (%.2fx ctrl)\n",f_no,(f_ctrl>.01f)?f_no/f_ctrl:1);
    printf("  Control:   %.1f\n",f_ctrl);
    printf("  Boost:     %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"COGNITIVE MAP WINS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
