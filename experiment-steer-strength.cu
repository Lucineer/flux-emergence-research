...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, float steer_strength, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(steer_strength>0){
            ...
            dx+=sx*steer_strength;dy+=sy*steer_strength;
...
int main(){
    float strengths[NSTRENGTHS]={0.0f,0.5f,1.0f,1.5f,2.0f,3.0f,5.0f};
    printf("=== STEERING STRENGTH: Law 234 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    for(int s=0;s<NSTRENGTHS;s++){
        float ts=0,tt=0;
        for(int tr=0;tr<TRIALS;tr++){
            cudaMemset(d_fa,1,FOOD*sizeof(int));
            simulate<<<blk,BLK>>>(d_s,d_a,d_fx,d_fy,d_fa,STEPS,N,FOOD,W,strengths[s],(unsigned int)(42+tr*1111+s*111));
        ...
        ts/=TRIALS;tt/=TRIALS;
        if(ts>best){best=ts;bi=s;}
        printf("Strength %-4.1f: score=%.3f fleet=%.1f\n",strengths[s],ts,tt);
    }
    printf("\nBest: strength %.1f (score=%.3f)\n",strengths[bi],best);
