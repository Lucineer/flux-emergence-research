/* experiment-gentle-niche.cu — Sustainable Niche Construction v2
   Very gentle depletion (0.02 per collection) with slow regrowth (0.01/tick).
   v39 used 0.15 depletion (too aggressive). v41 used 0.15 with 0.01 regrowth (still -15%).
   This uses 0.02 depletion — agents deplete 2x faster than regrowth, creating slow gradients. */
...
__global__ void regen(float*grid){int idx=blockIdx.x*blockDim.x+threadIdx.x;if(idx>=GRID*GRID)return;if(grid[idx]<0)grid[idx]+=REGROW;if(grid[idx]>0)grid[idx]=0;}
__global__ void tick_s(Agent*a,Resource*r,float*grid,int na,int nr,int t,int pt,int niche){
    ...
    if(br>=0&&bd<grab){
        ...
        if(niche){int gx=(int)(r[br].x*GRID)%GRID,gy=(int)(r[br].y*GRID)%GRID;atomicAdd(&grid[gy*GRID+gx],-DEPLETE);}
...
__global__ void respawn(Resource*r,float*grid,int nr,int t){int j=blockIdx.x*blockDim.x+threadIdx.x;if(j>=nr||!r[j].collected)return;if(t%20!=0)return;unsigned int s=(unsigned int)(j*2654435761u+t*9973);float nx=lcgf(&s),ny=lcgf(&s);r[j].x=nx;r[j].y=ny;r[j].collected=0;r[j].value=.5f+lcgf(&s)*.5f;}

int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  Experiment: Gentle Niche Construction\n");
    printf("  Deplete %.2f, regrow %.2f per tick (2:1 ratio)\n",DEPLETE,REGROW);
    printf("═══════════════════════════════════════════════════════\n\n");
    Agent*da,*ha,*db,*hb,*dc,*hc;Resource*dr;float*grid;
    cudaMalloc(&grid,GRID*GRID*sizeof(float));
    cudaMalloc(&da,NA*sizeof(Agent));cudaMalloc(&db,NA*sizeof(Agent));cudaMalloc(&dc,NA*sizeof(Agent));cudaMalloc(&dr,NR*sizeof(Resource));
    ...
    for(int e=0;e<5;e++){
        ...
        for(int t=0;t<MAXT;t++){regen<<<gblk,256>>>(grid);tick_s<<<blk,256>>>(da,dr,grid,NA,NR,t,250,1);respawn<<<rblk,256>>>(dr,grid,NR,t);cudaDeviceSynchronize();}
        ...
        for(int t=0;t<MAXT;t++){regen<<<gblk,256>>>(grid);tick_s<<<blk,256>>>(db,dr,grid,NA,NR,t,250,0);respawn<<<rblk,256>>>(dr,grid,NR,t);cudaDeviceSynchronize();}
        ...
        printf("Exp %d: niche=%.1f norm=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_niche/=5;f_norm/=5;f_ctrl/=5;
    float ratio=(f_norm>.01f)?f_niche/f_norm:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Niche:  %.1f (%.2fx ctrl)\n",f_niche,(f_ctrl>.01f)?f_niche/f_ctrl:1);
    printf("  Normal: %.1f (%.2fx ctrl)\n",f_norm,(f_ctrl>.01f)?f_norm/f_ctrl:1);
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Ratio:  %.2fx (%+.0f%%)\n",ratio,(ratio-1)*100);
    printf("  Verdict: %s\n",ratio>1.1?"NICHE WINS":ratio<0.9?"HURTS":"NO DIFFERENCE");
    printf("  (Compare: v39 0.15 deplete=-78%%, v41 0.15+0.01=-15%%)\n");
    printf("═══════════════════════════════════════════════════════\n");
