/* flux-emergence-v5.cu — Resource competition + generation.
   v4: roles had behavioral effects but eff still ~1.30x.
   Hypothesis: resources exhausted too fast (256 static, all collected early).
   v5: continuous resource generation, finite capacity per region,
       competition between archetypes for territory. */
...
typedef struct { float x, y, value; int collected; int spawn_tick; } Resource;
...
__global__ void init_resources(Resource *r, int n) {
    ...
    r[i].collected = 0; r[i].spawn_tick = 0;
}
...
__global__ void tick(Agent *a, Resource *r, int na, int nr, int t, int pt) {
    ...
    float explore_power = ag->role[0];
    float collect_power = ag->role[1];
    float detect_range = 0.1f + explore_power * 0.2f;
    float grab_range = 0.05f + collect_power * 0.05f;
...
__global__ void respawn_resources(Resource *r, int nr, int t, int interval) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= nr) return;
    if (r[i].collected && (t - r[i].spawn_tick) > interval) {
        unsigned int s = (unsigned int)(t * 2654435761u + i * 137u);
        r[i].x = lcgf(&s); r[i].y = lcgf(&s);
        r[i].value = 0.3f + lcgf(&s) * 0.7f;
        r[i].collected = 0; r[i].spawn_tick = t;
    }
}
...
int main() {
    printf("═══════════════════════════════════════════════════\n");
    printf("  FLUX EMERGENCE v5 — Resource Competition\n");
    printf("  512 resources, respawn after 50 ticks\n");
    printf("  Behavioral roles + anti-convergence + territory\n");
    printf("═══════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<N_EXP;e++){
        ...
        for(int t=0;t<MAX_TICKS;t++){
            ...
            respawn_resources<<<rblk,256>>>(dr,N_RESOURCES,t,50);
            cudaDeviceSynchronize();
        }
        ...
        /* Baseline: with respawn, agents should collect ~80% of capacity */
        float cap = (float)N_RESOURCES * 0.65f; /* higher bar */
        float eff = (cap > 0.01f) ? coord / cap : 1.0f;
