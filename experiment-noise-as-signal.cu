...
__global__ void phase2_exploit(float *scores, int *traces, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int grid_w, int use_traces, unsigned int seed) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(use_traces){
            int gx=(int)(x/w*grid_w)%grid_w;
            int gy=(int)(y/w*grid_w)%grid_w;
            // Look at neighboring cells, steer toward highest trace density
...
int main(){
    ...
    for(int mode=0;mode<2;mode++){
        ...
        for(int tr=0;tr<TRIALS;tr++){
            ...
            cudaMemcpy(d_fx,hfx,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            cudaMemcpy(d_fy,hfy,FOOD*sizeof(float),cudaMemcpyHostToDevice);
            // Phase 1: Fleet A leaves traces
            phase1_traces<<<blk,BLK>>>(d_tr,1500,NF,W,GRID,(unsigned int)(42+tr*1111));
            cudaDeviceSynchronize();
            // Phase 2: Fleet B exploits or ignores traces
            phase2_exploit<<<blk,BLK>>>(d_s,d_tr,d_fx,d_fy,d_fa,1500,NF,FOOD,W,GRID,mode,(unsigned int)(42+tr*1111));
            cudaDeviceSynchronize();
            float hs[NF];cudaMemcpy(hs,d_s,NF*sizeof(float),cudaMemcpyDeviceToHost);
            float avg=0;for(int i=0;i<NF;i++)avg+=hs[i];
            ts+=avg/NF;
        }
        ts/=TRIALS;
        printf("%-20s: score=%.3f\n",mode?"Uses traces":"Ignores traces",ts);
    }
    printf("\n>> Law 255: Can noise patterns be exploited as information?\n");
