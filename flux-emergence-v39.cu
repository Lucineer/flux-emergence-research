/* flux-emergence-v39.cu — Niche Construction (Overfishing)
   Novel: when a resource is collected, that location's spawn probability
   drops to 50% permanently. Agents literally reshape their environment.
   Tests niche construction — do agents create patterns of avoidance?
   Prediction: niche construction creates de facto territory through depletion. */
...
__global__ void tick_s(Agent*a,Resource*r,float*grid,int na,int nr,int t,int pt,int niche){
    ...
    if(br>=0&&bd<grab){
        ...
        /* v39: niche construction — deplete the grid cell */
        if(niche){
            int gx=(int)(r[br].x*GRID)%GRID,gy=(int)(r[br].y*GRID)%GRID;
            atomicAdd(&grid[gy*GRID+gx],-0.15f); /* deplete */
        }
...
/* Respawn resources using grid fertility */
__global__ void respawn_niche(Resource*r,float*grid,int nr,int t){
    int j=blockIdx.x*blockDim.x+threadIdx.x;if(j>=nr)return;
    if(!r[j].collected)return;
    if(t%25!=0)return; /* respawn check every 25 ticks */
    ...
    if(fert>-0.5f){ /* only spawn if not too depleted */
        r[j].x=nx;r[j].y=ny;r[j].collected=0;
        r[j].value=.5f+lcgf(&s)*.5f;
    }
}

__global__ void respawn_normal(Resource*r,int nr,int t){
    int j=blockIdx.x*blockDim.x+threadIdx.x;if(j>=nr)return;
    if(!r[j].collected)return;if(t%25!=0)return;
    unsigned int s=(unsigned int)(j*2654435761u+t*9973);
    r[j].x=lcgf(&s);r[j].y=lcgf(&s);r[j].collected=0;r[j].value=.5f+lcgf(&s)*.5f;
}

int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v39 — Niche Construction (Overfishing)\n");
    printf("  Collected locations become depleted, respawn resisted\n");
    printf("  A: niche construction  B: normal respawn  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    for(int e=0;e<5;e++){
        ...
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,grid,NA,NR,t,250,1);respawn_niche<<<rblk,256>>>(dr,grid,NR,t);cudaDeviceSynchronize();}
        ...
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(db,dr,grid,NA,NR,t,250,0);respawn_normal<<<rblk,256>>>(dr,NR,t);cudaDeviceSynchronize();}
        ...
        for(int t=0;t<MAXT;t++){tick_c<<<blk,256>>>(dc,dr,NA,NR,t,250);respawn_normal<<<rblk,256>>>(dr,NR,t);cudaDeviceSynchronize();}
    ...
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Boost:   %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"NICHE CONSTRUCTION WINS":boost<0.9?"DEPLETION HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
