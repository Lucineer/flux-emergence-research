...
__global__ void simulate(float *scores, int *alive, float *fx, float *fy, int *falive,
    int steps, int n, int food_count, int w, int wrap_mode, unsigned int seed) {
    ...
    for(int t=0;t<steps&&energy>0;t++){
        ...
        if(wrap_mode==0){
            // toroidal
            nx=fmodf(nx+w,w);ny=fmodf(ny+w,w);
        } else {
            // wall bounce
            if(nx<0){nx=-nx;dx=-dx;}
            if(nx>=w){nx=2*w-nx-1;dx=-dx;}
            if(ny<0){ny=-ny;dy=-dy;}
            if(ny>=w){ny=2*w-ny-1;dy=-dy;}
            if(nx<0)nx=0;if(nx>=w)nx=w-1;
            if(ny<0)ny=0;if(ny>=w)ny=w-1;
        }
        x=nx;y=ny;
        ...
        for(int i=0;i<food_count;i++){
            ...
            if(wrap_mode==0){if(fdx>w/2)fdx-=w;if(fdx<-w/2)fdx+=w;if(fdy>w/2)fdy-=w;if(fdy<-w/2)fdy+=w;}
...
int main(){
    const char* nm[]={"Toroidal","WallBounce"};
    printf("=== WORLD TOPOLOGY: Law 227 ===\nN=%d Food=%d Steps=%d Trials=%d\n\n",N,FOOD,STEPS,TRIALS);
    ...
    printf("\n>> Law 227: World topology effect on scripted agents\n");
